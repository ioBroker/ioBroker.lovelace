...
All files from the `cards` directory are included automatically. Press `Refresh overview` on the tab after an upload, then reload the browser - an adapter restart is not needed. Replacing a card with a newer file of the same name works as well, and the file selector deletes a card again.
