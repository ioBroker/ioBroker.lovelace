...
class WebServer {
    ...
    async _processSingleCall(ws, data, entity_id) {
        ...
        if (data.service.startsWith('select_option')) {
            let val;
            const target = data.service_data.option;
            if (entity.context.STATE.map) {
                val = Object.keys(entity.context.STATE.map).find(key => entity.context.STATE.map[key] === target);
            } else {
                //get list of options as fallback.
                const obj = await this.adapter.getForeignObjectAsync(id);
                if (obj.common.states) {
                    val = Object.keys(obj.common.states).find(key => obj.common.states[key] === target);
                }
            }
            if (!val && val !== 0) {
                val = target; //fallback.
            ...
            val = Number(val);
            this.log.debug(`${data.service}: ${id} = ${target} = ${val}`);
            await this.adapter.setForeignStateAsync(id, val, false, {user});
            this._sendResponse(ws, data.id);
        } else
                    ...
                    if (entity.context.STATE.getId === id) {
                        ...
                        if (entity.context.STATE.getParser) {
                            ...
                            if (entity.context.type === 'input_select') {
                                if (entity.context.STATE.map) {
                                    entity.attributes.initial = this._iobState2EntityState(id, entity.context.STATE.map[state.val] || state.val);
                                    entity.state = this._iobState2EntityState(id, entity.context.STATE.map[state.val] || state.val);
                                }
                            } else {
