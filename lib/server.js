...
function setJsonAttribute(obj, path, value, log) {
    if (!path) {
        log && log.error('Invalid attribute name for ' + JSON.stringify(obj) + ' = ' + value);
        return;
    }
            ...
            if (somethingFound) {
                ...
                if (state && state.id) {
                    ...
                    entity.context.ATTRIBUTES.push({
                        ...
                        getId: dayShiftId,
                        getState: (entity, attr) => {
                            const date = new Date();
                            attr.dayShift && date.setDate(date.getDate() + attr.dayShift);
                            setJsonAttribute(entity.attributes, attr.attribute, date.toISOString(), this.log);
                        }
                    });
                }
    ...
    _processMediaPlayer(id, control, name, room, func, _obj, objects) {
            ...
            if (objects[state.id] && objects[state.id].common && objects[state.id].common.write) {
                playPauseAttribute = {
                    ...
                    parseCommand: (entity, command, params, user) => new Promise((resolve, reject) =>
                        this.adapter.setForeignState(entity.setId, !entity.state, {user}, err =>
        ...
        if (!playPauseAttribute) {
            ...
            if (state && state.id) {
                playPauseAttribute = {
                    ...
                    playId: state.id,
                    parseCommand: (entity, command, params, user) => new Promise((resolve, reject) =>
                        this.adapter.setForeignState(entity.state === 'playing' ? command.pauseId : command.playId, true, {user}, err =>
                            err ? reject(err) : resolve()))
                };
            ...
            entity.context.COMMANDS.push({
                ...
                parseCommand: (entity, command, params, user) =>
                    new Promise((resolve, reject) =>
                        this.adapter.setForeignState(command.setId, true, {user}, err =>
                            err ? reject(err) : resolve()))
        ...
        if (state && state.id) {
            ...
            entity.context.COMMANDS.push({
                ...
                parseCommand: (entity, command, params, user) => new Promise((resolve, reject) => {
                    ...
                    this.adapter.getForeignState(command.getId, (err, state) => {
                        ...
                        this.adapter.setForeignState(command.setId, val, {user}, err =>
                            err ? reject(err) : resolve());
                    });
                })
            });
            ...
            entity.context.COMMANDS.push({
                ...
                parseCommand: (entity, command, data, user) =>
                    new Promise((resolve, reject) =>
                        this.adapter.setForeignState(command.setId, data.service_data.value !== undefined ? data.service_data.value : data.service_data.volume_level, {user}, err =>
                            err ? reject(err) : resolve()))
            });
        ...
        if (state && state.id) {
            ...
            entity.context.COMMANDS.push({
                ...
                parseCommand: (entity, command, data, user) =>
                    new Promise((resolve, reject) =>
                        this.adapter.setForeignState(command.setId, data.service_data.is_volume_muted, {user}, err =>
                            err ? reject(err) : resolve()))
            });
        ...
        if (state && state.id) {
            ...
            entity.context.COMMANDS.push({
                ...
                parseCommand: (entity, command, params, user) =>
                    new Promise((resolve, reject) =>
                        this.adapter.setForeignState(command.setId, false, {user}, err =>
                            err ? reject(err) : resolve()))
            });
        ...
        return new Promise((resolve, reject) => {
            ...
            if (entity.context.STATE.setId && entity.context.STATE.getId) {
                ...
                this.adapter.getForeignState(entity.context.STATE.getId, {user}, (err, state) => {
                    ...
                    if (!state || !state.val) {
                        // ON
                        this.adapter.setForeignState(entity.context.STATE.setId, true, {user}, () => {
                            // If dimmer level set
                            if (data.data_servivce.brightness_pct !== undefined) {
                                this.adapter.setForeignState(command.setId, data.data_servivce.brightness_pct, {user}, err =>
                                    err ? reject(err) : resolve());
                            } else {
                                resolve();
                            }
    ...
    _processThermostat(id, control, name, room, func, _obj, objects) {
        ...
        if (state && state.id) {
            ...
            entity.context.COMMANDS = [{
                ...
                parseCommand: (entity, command, data, user) =>
                    new Promise((resolve, reject) =>
                        this.adapter.setForeignState(command.setId, data.service_data.temperature, {user}, err =>
                            err ? reject(err) : resolve()))
            }];
            ...
            if (entityType === 'light') {
                if (obj.common.type === 'number') {
                    ...
                    entity.context.COMMANDS = [{
                        ...
                        parseCommand: (entity, command, data, user) =>
                            new Promise((resolve, reject) =>
                                this.adapter.setForeignState(command.setId, command.off, {user}, err => err ? reject(err) : resolve()))
                    }];
                }
                    ...
                    if (ids && ids.length) {
                        ...
                        ids.forEach((id, i) => {
                            ...
                            if (states[id] && states[id].val !== undefined) {
                                setJsonAttribute(entity.attributes, attribute, this._iobState2EntityState(null, states[id].val, attribute), this.log);
                            } else {
                                setJsonAttribute(entity.attributes, attribute, 'unknown', this.log);
                            }
                ...
                if (entity.context.ATTRIBUTES) {
                    ...
                    if (attr) {
                        ...
                        if (attr.getParser) {
                            ...
                            setJsonAttribute(entity.attributes, attr.attribute, this._iobState2EntityState(null, state.val, attr.attribute), this.log);
                        }
                    } else {
                        return;
                    }
