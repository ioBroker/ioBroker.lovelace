    ...
    _processBlind(id, control, name, room, func, _obj, objects) {
        ...
        if (state && state.id) {
            ...
            entity.context.STATE.getParser = (entity, attr, state)=> {
                ...
                if (state.val != null) {
                    entity.attributes.current_position = ((state.val - entity.attributes.iobOpen) / entity.attributes.iobClose) * 100.0;
                ...
                parseCommand: (entity, command, data, user) => {
                    return new Promise((resolve, reject) => {
                        if (data.service_data.position >= 0) {
                            const value = (data.service_data.position / 100) * entity.attributes.iobClose + entity.attributes.iobOpen;
                            ...
                            if (attr && attr.setId) {
                                const value = (data.service_data.position / 100) * attr.close + attr.open;
            ...
            entity.context.ATTRIBUTES = [{
                ...
                getParser: (entity, attr, state) => {
                    ...
                    if (state.val != null) {
                        ...
                        entity.attributes.current_tilt_position = ((state.val - attr.open) / attr.close) * 100.0;
                    }
                }
            }];
            this._addID2entity(state.id, entity);
        ...
        const haveUp = this._addCommand(entity, control, 'UP', 'open_cover', 1, entity => entity.state = 'opening');
        const haveDown = this._addCommand(entity, control, 'DOWN', 'close_cover', 2, entity => entity.state = 'closing');
        ...
        //manually go up/down in case there is no button.
        if (!haveUp) {
            const command = {
                service: 'open_cover',
                setId: entity.context.STATE.setId,
