...
class WebServer {
    constructor(options) {
        ...
        this.converter = {
            ...
            [Types.blindSlider]:            this._processBlind.bind(this),
            [Types.blindButtons]:           this._processBlind.bind(this),
    ...
    _addBlindLevel(entities, control, objects, name, id, room, func, _obj) {
    ...
    _addTiltLevel(entity, control, objects, name) {
        //support for tilt %:
        const state = control.states.find(s => s.id && s.name === name);
        if (state && state.id) {
            const common = objects[state.id] && objects[state.id].common ? objects[state.id].common : {};
    ...
    _processBlind(id, control, name, room, func, _obj, objects) {
        ...
        if(this._addBlindLevel(entities, control, objects,'SET', id, room, func, _obj)) {
            entity.context.STATE.invert = !!this.adapter.config.blindsInvert;
        } else if (this._addBlindLevel(entities, control, objects, 'SET_OPEN', id, room, func, _obj)) {
            entity.context.STATE.invert = false;
        } else if (this._addBlindLevel(entities, control, objects, 'SET_CLOSE', id, room, func, _obj)) {
            entity.context.STATE.invert = true;
        }

        if(this._addTiltLevel(entity, control, objects, 'TILT_SET')) {
            const attr = entity.context.ATTRIBUTES.find(a => a.attribute === 'tilt');
            attr.invert = true;
        } else if(this._addTiltLevel(entity, control, objects, 'TILT_OPEN_SET')) {
            const attr = entity.context.ATTRIBUTES.find(a => a.attribute === 'tilt');
            attr.invert = false;
        } else if(this._addTiltLevel(entity, control, objects, 'TILT_CLOSE_SET')) {
            const attr = entity.context.ATTRIBUTES.find(a => a.attribute === 'tilt');
            attr.invert = true;
        }
    ...
    _processManualEntity(id, _entities, cb) {
        this.adapter.getForeignObject(id, (err, obj) => {
            ...
            const name = replaceInvalidChars(obj.common.custom[this.adapter.namespace].name || this._generateName(obj, 'en'));
    ...
    _generateName(obj, lang) {
        return this._getObjectName(obj, lang).replace(/[^-._\w0-9А-Яа-яÄÜÖßäöü ]/g, ' ');
    }
