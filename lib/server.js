...
class WebServer {
    ...
    _setLightAttributesToIOBStates(data, entity, user, resolve, reject) {
        ...
        if (data.service_data.hs_color) {
            ...
            if (attr) {
                if (attr.is_rgb_string || attr.is_rgb_values) {
                    ...
                    let v = (entity.attributes.brightness / 255 * 100) || entity.attributes.brightness_pct;
                    if (v === undefined) {
                        v = 100; //assume 100 if no brightness at all. if (v) will be false for v === 0.
                    }
                    const rgb = this._HSV2RGB(h, s, v);
                    this.adapter.log.debug('RGB: ' + JSON.stringify(rgb) + ' from [' + h + ',' + s + ',' + v + ']');
                    ...
                    this.adapter.setForeignState(attr.setId, h, false, {user}, err => {
                        if (err) {
                            reject(err);
                        } else {
                            if (attr_Sat) {
                                this.adapter.setForeignState(attr_Sat.setId, s, false, {user}, err => err ? reject(err) : resolve());
                            } else {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (entity.context.STATE.setId && entity.context.STATE.getId) {
                ...
                this.adapter.getForeignStates(ids, {user}, (err, state) => {
                    ...
                    if (!state[entity.context.STATE.getId] || !state[entity.context.STATE.getId].val) {
                        // turn ON:
                        this.adapter.setForeignState(entity.context.STATE.setId, true, false, {user}, () => this._setLightAttributesToIOBStates(data, entity, user,resolve, reject));
                    } else {
                        this._setLightAttributesToIOBStates(data, entity, user,resolve, reject);
                    }
                });
            } else {
                this._setLightAttributesToIOBStates(data, entity, user,resolve, reject);
    ...
    _getLightStateControl(control) {
        ...
        if (!state) { //for dimmer, needs to be before 'SET' -> because in dimmer set is for level.
            state = control.states.find(s => s.id && s.name === 'ON_SET');
        }
        if (!state) {
            state = control.states.find(s => s.id && s.name === 'SET');
        }
        if (!state) {
            state = control.states.find(s => s.id && s.name === 'ON_LIGHT');
        }
        return state;
    }

    _addLightState(control, state, entity) {
    ...
    _addHueAndSat(control, objects, entity) {
        ...
        if (sat && sat.id) {
            ...
            const attribute = {
                ...
                getParser: (entity, attr, state) => {
                    state = state || {val: 0};
                    entity.attributes.hs_color[1] = state.val;
                }
            };
    ...
    _addRGBSingle(control, objects, entity) {
        ...
        if (state && state.id) {
            const attribute = {
                ...
                getParser: (entity, attr, state) => {
                    ...
                    entity.attributes.brightness_pct = hsv[2];
                    entity.attributes.brightness = hsv[2] / 100 * 255; //255 is ok here, HASS world.
                }
            };
    ...
    _addRGB(control, objects, entity) {
        if (entity.context.ATTRIBUTES && entity.context.ATTRIBUTES.find((a) => a.attribute === 'hs_color')) {
        ...
        if (redState && redState.id && greenState && greenState.id && blueState && blueState.id) {
            ...
            const attribute = {
                ...
                getParser: (entity, attr, state) => {
                    ...
                    const hsv = [entity.attributes.hs_color[0], entity.attributes.hs_color[1], entity.attributes.brightness_pct || entity.attributes.brightness / 255 * 100 ||100];
                    ...
                    entity.attributes.brightness_pct = hsv[2];
                    entity.attributes.brightness = hsv[2] / 100 * 255; //255 is ok here, HASS world.
                    this.log.debug(entity.entity_id  + ' got ' + JSON.stringify(hsv_new) + ' from Red ' + r + ' and ' + JSON.stringify(rgb) + ' and old hsv ' + JSON.stringify(hsv));
                }
            };
            const attributeGreen = {
                ...
                getParser: (entity, attr, state) => {
                    ...
                    entity.attributes.brightness_pct = hsv[2];
                    entity.attributes.brightness = hsv[2] / 100 * 255; //255 is ok here, HASS world.
                    this.log.debug(entity.entity_id  + ' got ' + JSON.stringify(hsv_new) + ' from Green ' + g + ' and ' + JSON.stringify(rgb) + ' and old hsv ' + JSON.stringify(hsv));
                }
            };
            const attributeBlue = {
                attribute: 'hs_blue',
                ...
                getParser: (entity, attr, state) => {
                    ...
                    entity.attributes.brightness_pct = hsv[2];
                    entity.attributes.brightness = hsv[2] / 100 * 255; //255 is ok here, HASS world.
                    this.log.debug(entity.entity_id  + ' got ' + JSON.stringify(hsv_new) + ' from Blue ' + b + ' and ' + JSON.stringify(rgb) + ' and old hsv ' + JSON.stringify(hsv));
                }
            };
    ...
    _processLightAdvanced(id, control, name, room, func, _obj, objects) {
        const state = this._getLightStateControl(control);
        if (state && state.id) {
            this.adapter.log.debug(control.type + ' - Controls: ' + JSON.stringify(control, null, 2));

            const entity = this._processCommon(id, name, func, room, _obj, 'light');

            //fill in on/off state id.
            this._addLightState(control,state, entity);
            ...
            this._addRGBSingle(control, objects, entity);

            //add rgb as single states. Will only happen if no hue and no rgbSingle:
            this._addRGB(control, objects, entity);
            ...
            this.log.warn('Could not add ' + id + ' of type ' + control.type + ' -> no on/off control found.');
            return null;
        }
    }
                    ...
                    if (this.converter[control.type]) {
                        ...
                        if (entities && entities.length) {
                            ...
                            entities.forEach(entity => {
                                ...
                                if (_entity) {
                                    this.log.warn('Duplicates found for ' + entity.entity_id);
                                    return;
                                }
