...
class WebServer {
    ...
    _processLocation(id, control, name, room, func, _obj, objects) {
        // - geo_location => attributes: [latitude, longitude, passive, icon, radius, entity_picture, gps_accuracy, source]
    ...
    _processWindow(id, control, name, room, func, _obj, objects) {
        ...
        let state = control.states.find(s => s.id && s.name === 'ACTUAL');
        entity.context.STATE = {getId: null};
        entity.attributes.icon = 'mdi:window-maximize';
        if (state && state.id) {
            entity.context.STATE.getId = state.id;
    ...
    _renderIndex() {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (lines[i].trim().match(/<\/body>/)) {
                ...
                // Collect all custom cards
                filesWalk(__dirname + '/' + CUSTOM_DIR, fileName => {
                    if (fileName.endsWith('.js')) {
                        nLines.push('<script type="text/javascript" src="' + fileName.substring(CUSTOM_DIR.length).replace(/\\/g, '/') + '"></script>');
                    }
                });
            }
    ...
    _init() {
        ...
        this._app.get('/cards/', express.static(__dirname + '/' + CUSTOM_DIR));
