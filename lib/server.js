...
const getFriendlyName        = require('./converters/friendly_name').getFriendlyName;
...
class WebServer {
    ...
    async onStateChange(id, state, forceUpdate = false) {
        if (state) {
            ...
            await this._conversation.onStateChange(id, state, this._wss);
        }
    ...
    async _initConnection(ws) {
        ...
        ws.on('message', async message => {
            ...
            if (message.type === 'config/entity_registry/list_for_display') {
                this._sendResponse(ws, message.id, {entities: entityData.entities});
            } else
