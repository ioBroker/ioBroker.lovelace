...
class WebServer {
    ...
    _processMediaPlayer(id, control, name, room, func, _obj, objects) {
        ...
        if (state && state.id) {
            ...
            entity.context.STATE.getParser = (entity, attr, state) => {
                ...
                this.adapter.log.debug('Playstate changed ' + state.val + ' => ' + entity.state);
            };
    ...
    async _updateDevices() {
        const data = await this._readObjects();
    ...
    async _readObjects() {
        ...
        try {
            ...
            const _states = this.adapter.getObjectViewAsync('system', 'state', params);
            const _channels = this.adapter.getObjectViewAsync('system', 'channel', params);
            const _devices = this.adapter.getObjectViewAsync('system', 'device', params);
            const _enums = this.adapter.getObjectViewAsync('system', 'enum', {});
