...
class WebServer {
    constructor(options) {
        ...
        this.converter = {
            ...
            [Types.rgbSingle]:              this._processRGBSingle.bind(this),
    ...
    _parseLightOn(entity, command, data, user) {
            ...
            if (data.service_data.hs_color) {
                ...
                if (attr) {
                    if (attr.is_rgb_string) {
                        ...
                        let v = entity.attributes.brightness / 255 || entity.attributes.brightness_pct;
                        if (v === undefined) {
                        ...
                        adapter.log.debug('attr: ' + JSON.stringify(attr, null, 2));
                        adapter.log.debug('generated rgbString ' + rgbString + ' from [' + r + ',' + g + ',' + b + '], [' + h + ',' + s + ',' + v + '], c=' + c + ', x=' + x + ', m='+m);
                        adapter.setForeignState(attr.setId, rgbString, false, {user}, err => err ? reject(err) : resolve());
                    } else {
                        adapter.setForeignState(attr.setId, h, false, {user}, err => {
                            if (err) {
                                ...
                                if (attr_Sat) {
                                    adapter.setForeignState(attr_Sat.setId, s, false, {user}, err => err ? reject(err) : resolve());
                                } else {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (entity.context.STATE.setId && entity.context.STATE.getId) {
                ...
                const ids = [];
                ids.push(entity.context.STATE.getId);
                //get states of attribute states. -> I do not really use them right now. Should I?
                /*const serviceIds = entity.context.ATTRIBUTES.map(entry => {
                    if (entry.attribute === 'color_temp' || entry.attribute === 'brightness' || entry.attribute === 'hs_color' || entry.attribute === 'hs_saturation') {
                        return entry.setId;
                    }
                    return '';
                });
    ...
    _addOnOff(control, objects, entity) {
        ...
        if (state && state.id) {
            ...
            this.log.warn('No on/off state found for ' + entity.entity_id + ' and type ' + control.type);
            return false;
        }
    }
    ...
    _addHueAndSat(control, objects, entity) {
        ...
        if (sat && sat.id) {
            if (!hue || !hue.id) {
                this.log.error('Saturation present but no hue id found for ' + sat.id + '. Hue won\'t work.');
            }
            const attribute = {
            ...
            this.log.error('Hue present but no saturation id found for ' + hue.id + '. Saturation won\'t work.');
        }
    }

    _addRGBSingle(control, objects, entity) {
        if (entity.context.ATTRIBUTES && entity.context.ATTRIBUTES.find((a) => a.attribute === 'hs_color')) {
            this.log.info('Color already present, skip RGB color.');
            return;
        }
    ...
    _processColorTemperature(id, control, name, room, func, _obj, objects) {
        ...
        if(!this._addOnOff(control, objects, entity)) {
            this.log.warn('Could not add ' + id + ' of type ct -> no on/off found.');
            return;
        }
    ...
    _processHue(id, control, name, room, func, _obj, objects) {
        ...
        //fill in on/off state id.
        if(!this._addOnOff(control, objects, entity)) {
            this.log.warn('Could not add ' + id + ' of type hue -> no on/off found.');
            return;
        }
    ...
    _processRGBSingle(id, control, name, room, func, _obj, objects) {
        const entity = this._processCommon(id, name, func, room, _obj, 'light');
        ...
        //add hue and sat:
        //this._addHueAndSat(controlHue, objects, entity);
