...
class WebServer {
    ...
    _markForUpdate(id, obj, entity) {
        if (entity) { //we have entity -> keep only on change for one entity, because it will be completely rebuild.
            const foundIndex = this._objectData.updatedObjects.findIndex(e => e.entity && e.entity === entity.entity_id);
            if (foundIndex !== -1) {
            ...
            const foundIndex = this._objectData.updatedObjects.findIndex(e => e.id === id);
            if (foundIndex !== -1) {
