...
class WebServer {
    ...
    async onObjectChange(id, obj) {
        if (obj) {
                ...
                if (id.startsWith('enum.rooms.')) {
                    ...
                    for (const entity of affectedEntities) {
                        if (entity) {
                            ...
                            this._objectData.updatedObjects.push({
                                id: entity.context.id,
                                obj: this._objectData.objects[entity.context.id],
                                entity
                            });
                        }
                    }
                    for (const id of ids) {
                        this._objectData.updatedObjects.push({id, obj: this._objectData.objects[id]});
                    }
            ...
            const foundIndex = this._objectData.ids(id);
        ...
        if (id === this.adapter.namespace + '.configuration') {
            ...
            if (affectedEntities && affectedEntities.length) {
                for (const entity of affectedEntities) {
                    this.log.debug(`${id} changed, ${entity.entity_id} affected.`);
                    this._objectData.updatedObjects.push({id: entity.context.id, obj: this._objectData.objects[entity.context.id], entity});
                }
            } else if (obj) {
                this._objectData.updatedObjects.push({id, obj}); //no old entities affected.
            }
