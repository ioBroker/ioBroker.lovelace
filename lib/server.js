...
const getHistory             = require('./modules/history').getHistory;
...
class WebServer {
    ...
    async _processManualEntity(id, forcedEntityId) {
        try {
            ...
            if (entityType === 'light') {
                ...
                const state = await this.adapter.getForeignStateAsync(id);
                entity.attributes.initial = iobState2EntityState(entity, state ? state.val : undefined);
            }
    ...
    _init() {
        ...
        this._app.get('/api/history/period/:start', async (req, res) => {
            ...
            const result = await getHistory(this.adapter, entities, req.params.start, req.query.end_time, req.query.noAttributes, req._user || this.config.defaultUser);
            res.json(result);
        });
