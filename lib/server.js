...
// (binary_sensor, switch, lock, sensor, geo_location, camera, weather, cover are registered
//  transitively when converter.ts is loaded above — these explicit requires are only
//  needed for processManualEntity access or to ensure the build module is loaded.)
...
class WebServer {
    ...
    async _processManualEntity(id) {
        try {
            ...
            const entity_id = utils.createEntityNameFromCuston(obj, this.adapter.namespace); //maybe use forced id if no id set in object... but does that happen at all?
