    ...
    _parseCommandDimmer(entity, command, data, user) {
        return new Promise((resolve, reject) => {
            ...
            if (entity.context.STATE.setId && entity.context.STATE.getId) {
                ...
                const bright = entity.context.ATTRIBUTES.map(entry => {if (entry.attribute = "brightness") return entry.getId; else return ''});
                if (bright && bright.length) {ids.push(bright[0])};
                //const gIds = ids;
                    ...
                    if (!state[entity.context.STATE.getId] || !state[entity.context.STATE.getId].val) {
                        ...
                        this.adapter.setForeignState(entity.context.STATE.setId, true, false, {user}, () => {
                            ...
                            if (data.service_data.brightness !== undefined) {
                                ...
                                } else   entity.attributes.brightness = command.on * 255 / entity.attributes.iob_max;
    ...
    onStateChange(id, state) {
        ...
        this.templates.forEach(t => {
            if (t.ids.includes(id)) {
                this.templateStates[id] = state || null;
                const t = {
                    id: t.id,
                    type: 'event',
                    event: {
    ...
    _initConnection(ws) {
        ...
        ws.on('message', message => {
            ...
            if (message.type === 'render_template') {
                ...
                const obj = {template, ids: [], id: message.id, ws};
        ...
        ws.on('close', () => {
            const t = this.templates.find(t => t.ws === ws);
            const pos = this.templates.indexOf(t);
            if (pos !== -1) {
