...
class WebServer {
    ...
    async _processManualEntity(id) {
        try {
            ...
            if (entityType === 'light') {
                return converterLight.processManualEntity(id, obj, entity, this._objectData.objects);
            } else if(entityType === 'input_datetime') {
                return converterDatetime.processManualEntity(id, obj, entity, this._objectData.objects);
                ...
                return converterAlarmCP.processManualEntity(id, obj, entity, this._objectData.objects);
    ...
    async _initThemes() {
        ...
        const darkSate = this.adapter.getStateAsync(this.adapter.namespace + '.control.themeDark');
        //remember currently selected theme, if valid. Select default otherwise.
        ...
        if (id === this.adapter.namespace + '.configuration') {
            ...
            this._updateTimer = setTimeout(async () => {
                ...
                while (this._objectData.updatedObjects.length > 0) {
                    ...
                    //object deleted:
                    if (!obj) {
                        needUpdate = utils.removeIdFromEntities(id) || needUpdate; //handles remove
                        if (needUpdate) {
