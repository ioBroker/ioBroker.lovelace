    ...
    async onObjectChange(id, obj) {
        ...
        if (id === `${this.adapter.namespace}.configuration`) {
            ...
            this._updateTimer = setTimeout(async () => {
                ...
                const promises = [];
                let needUpdate = [];
                this.log.debug(`Update timer expired, ${this._objectData.updatedObjects.length} update tasks collected.`);
                const idsWithUpdate = [];
                ...
                while (this._objectData.updatedObjects.length > 0) {
                    ...
                    for (const e of entities) {
                        this.log.debug(`Object ${id} changed, update of ${e?.entity_id} done, storing.`);
                        for (let i = this._objectData.updatedObjects.length - 1; i >= 0; i -= 1) {
                            const updateTask = this._objectData.updatedObjects[i];
                            if (updateTask.entity && updateTask.entity.entity_id === e.entity_id) {
                                this.log.debug(
                                    `Removing ${e.entity_id} from update list, because just updated now for ${id}.`,
                                );
                                this._objectData.updatedObjects.splice(i, 1);
                            }
                        }
                        needUpdate.push(e);
