...
class WebServer {
    ...
    _processBlind(id, control, name, room, func, _obj, objects) {
        ...
        if (state && state.id) {
            ...
            entity.attributes.iobClose = common.min || 0;
            entity.attributes.iobOpen = common.max || 100;
            entity.context.STATE.getParser = function (entity, attr, state) {
                if (!state) state = {val: null};
                if (state.val != null) {
                    //TODO: scale. 0 = closed, 100 = fully open. -> my blinds are the otherway round. Is this always the case in ioBroker?
                    entity.attributes.current_position = (1.0 - ((state.val - entity.attributes.iobClose) / entity.attributes.iobOpen)) * 100.0;
            ...
            entity.context.COMMANDS.push({
                ...
                setId: state.id,
                parseCommand: function (entity, command, data, user) {
                    return new Promise((resolve, reject) => {
                        if (data.service_data.position >= 0) {
                            const value = (1.0 - (data.service_data.position / 100)) * entity.attributes.iobOpen + entity.attributes.iobClose;
                            this.adapter.setForeignState(command.setId, value, false,
                            ...
                            if (attr && attr.setId) {
                                const value = (1.0 - (data.service_data.position / 100)) * attr.open + attr.close;
            ...
            entity.context.ATTRIBUTES = [{
                ...
                getParser: (entity, attr, state) => {
                    ...
                    if (state.val != null) {
                        ...
                        entity.attributes.current_tilt_position = (1.0 - ((state.val - attr.close) / attr.open)) * 100.0;
                    }
                }
            }];
            this._addID2entity(state.id, entity);
        ...
        this._addCommand(entity, control, 'UP', 'open_cover', 1, entity => entity.state = 'opening');
        this._addCommand(entity, control, 'DOWN', 'close_cover', 2, entity => entity.state = 'closing');
