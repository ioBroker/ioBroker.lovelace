...
class WebServer {
        ...
        this.adapter.getForeignObjectAsync('system.config')
            ...
            .then(() => {
                ...
                try {
                    try {
                        ...
                        if (depError.message.includes('yaml.safeLoad') && depError.message.includes('removed')) {
                            this._tehems = yaml.load(this.config.themes || '') || {};
                        } else {
                            throw depError;
                        }
                    }
                ...
                return this.adapter.extendObjectAsync(this.adapter.namespace + '.control.theme', {common: {states}})
                    .then(() => this.adapter.extendObjectAsync(this.adapter.namespace + '.control.themeDark', { common: { states: states}}));
            })
            ...
            .then(state => {
                ...
                if (state && (this._themes[state.val] || state.val === 'default')) {
                    ...
                    this._currentThemeDark = this.config.defaultThemeDark || 'default';
                    this.adapter.setStateAsync(this.adapter.namespace + '.control.theme', this._currentThemeDark, true);
                }
            })
            .then(() => {
                // check every minute
            ...
            if (id === this.adapter.namespace + '.control.shopping_list') {
                ...
                return !state.ack &&
                    this._clearNotification(state.val).then(() =>
                        this._sendUpdate('persistent_notifications_updated'));
            } else if (id === this.adapter.namespace + '.control.theme' || this.adapter.namespace + '.control.themeDark') {
    ...
    async _listFiles() {
        try {
            ...
            this.adapter.warn('Could not load custom cards: ' + err);
        }
    }
    ...
    async _init() {
        ...
        this._app.use('/adapter/', async (req, res) => {
            ...
            try {
                ...
                if (image.buffer === null || image.buffer === undefined) {
                    throw new Error('File empty');
                } else {
                    image.mimeType = image.mimeType || (mime.getType || mime.lookup).call(mime, url) || 'text/javascript';
                        ...
                        if (val.startsWith('/adapter/')) {
                            ...
                            if (image) {
                                if (image.buffer === null || image.buffer === undefined) {
                                    ...
                                    image.mimeType = image.mimeType || (mime.getType || mime.lookup).call(mime, url) || 'text/javascript';
                                    result = {content_type: image.mimeType, content: image.buffer.toString('base64')};
                                }
                            }
                        } else {
