...
class WebServer {
    ...
    _parseLightOn(entity, command, data, user) {
        ...
        function setBrightnessOrCt(resolve, reject) {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (entity.context.STATE.setId && entity.context.STATE.getId) {
                ...
                const serviceIds = entity.context.ATTRIBUTES.map(entry => {
                    if (entry.attribute === 'color_temp' || entry.attribute === 'brightness') {
                        return entry.setId;
                    }
                    return '';
                });
                ...
                this.adapter.getForeignStates(ids, {user}, (err, state) => {
                    ...
                    if (!state[entity.context.STATE.getId] || !state[entity.context.STATE.getId].val) {
                        // turn ON:
                        this.adapter.setForeignState(entity.context.STATE.setId, true, false, {user}, () => setBrightnessOrCt(resolve, reject));
                    } else {
                        setBrightnessOrCt(resolve, reject);
                    }
                });
            } else {
                setBrightnessOrCt(resolve, reject);
    ...
    _addBrightness(control, objects, entity) {
        ...
        if (state && state.id) {
            ...
            this.log.debug('CT: Found brightness state ' + dimmerId + ' - adding it.');
            entity.attributes.supported_features |= 0x01;
            const obj = objects[dimmerId];
            if (obj.common.max) {
                attribute.obj.common.max;
            } else {
