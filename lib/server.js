...
class WebServer {
    ...
    _addCommand(entity, control, searchString, serviceName, featureNumber, additionalParsing) {
    ...
    _processBlind(id, control, name, room, func, _obj, objects) {
        ...
        entity.context.COMMANDS = [];
        entity.attributes.device_class = 'shade'; //use shade, because it does not support tilt, which is probably what we have.
        entity.context.STATE = {setId: null, getId: null};
        ...
        entity.attributes.iobClose = 100;
        entity.attributes.iobOpen = 0;
        let state = control.states.find(s => s.id && s.name === 'SET');
        if (state && state.id) {
            ...
            entitySlider = this._processCommon(id, name, room, func, _obj, 'input_number');
            ...
            entity.context.STATE.getId = state.id;
            this._addID2entity(state.id, entity);
            entity.attributes.iobClose = common.max || 100;
            entity.attributes.iobOpen = common.min || 0;
            entity.context.STATE.getParser = (entity, attr, state)=> {
                if (!state) state = {val: null};
                if (state.val != null) {
                    entity.attributes.current_position = 100 - (((state.val - entity.attributes.iobOpen) / entity.attributes.iobClose) * 100.0);
                    if (state.val === entity.attributes.iobClose) {
                        entity.state = 'closed';
                    } else if (state.val === entity.attributes.iobOpen) {
                        entity.state = 'open';
                    } else {
                        entity.state = null;
                    }
                ...
                parseCommand: (entity, command, data, user) => {
                    return new Promise((resolve, reject) => {
                        if (data.service_data.position >= 0) {
                            const value = ((100 - data.service_data.position) / 100) * entity.attributes.iobClose + entity.attributes.iobOpen;
                            ...
                            if (attr && attr.setId) {
                                const value = ((100 - data.service_data.position) / 100) * attr.close + attr.open;
                    ...
                    return new Promise((resolve, reject) => {
                        ...
                        if (entity.state === 'open' || entity.state === 'opening') {
                            if (down) {
                                ...
                                this.adapter.setForeignState(command.setId, entity.attributes.iobClose, false,
                                    {user}, err => err ? reject(err) : resolve());
                            }
                        } else {
                            if (up) {
                                ...
                                this.adapter.setForeignState(command.setId, entity.attributes.iobOpen, false,
                                    {user}, err => err ? reject(err) : resolve());
                            }
                        }
                    });
        ...
        //support get/set in two different states:
        state = control.states.find(s => s.id && s.name === 'ACTUAL');
        if (state && state.id) {
            entity.context.STATE.getId = state.id;
            this._addID2entity(state.id, entity);

            entitySlider.context.STATE.getId = state.id;
            this._addID2entity(state.id, entitySlider);
        }

        //support for tilt %:
        state = control.states.find(s => s.id && s.name === 'TILT_SET');
        if (state && state.id) {
            const common = objects[state.id] && objects[state.id].common ? objects[state.id].common : {};
            entity.context.ATTRIBUTES = [{
                attribute: 'tilt',
                ...
                setId: state.id,
                open: common.min || 0,
                close: common.max || 100,
                getParser: (entity, attr, state) => {
                    if (!state) state = {val: null};
                    if (state.val != null) {
                        //is tilt this way in ioBroker?
                        entity.attributes.current_tilt_position = 100 - (((state.val - attr.open) / attr.close) * 100.0);
                    }
                }
            }];
            this._addID2entity(state.id, entity);
        ...
        state = control.states.find(s => s.id && s.name === 'TILT_ACTUAL');
        if (state && state.id) {
            entity.context.STATE.getId = state.id;
            this._addID2entity(state.id, entity);
        }
        ...
        this._addCommand(entity, control, 'STOP', 'stop_cover', 8);
        const haveOpen = this._addCommand(entity, control, 'OPEN', 'open_cover', 1, entity => entity.state = 'opening');
        const haveClose = this._addCommand(entity, control, 'CLOSE', 'close_cover', 2, entity => entity.state = 'closing');
        this._addCommand(entity, control, 'TILT_OPEN', 'open_cover_tilt', 16);
        this._addCommand(entity, control, 'TILT_CLOSE', 'close_cover_tilt', 32);
        this._addCommand(entity, control, 'TILT_STOP', 'stop_cover_tilt', 64);
        ...
        if (!haveOpen) {
            const command = {
                ...
                parseCommand: (entity, command, params, user) => new Promise((resolve, reject) => {
                    ...
                    this.adapter.setForeignState(command.setId, entity.attributes.iobOpen, false, {user}, err =>
                        err ? reject(err) : resolve());
                })
            };
            entity.context.COMMANDS.push(command);
        ...
        if (!haveClose) {
            const command = {
                ...
                parseCommand: (entity, command, params, user) => new Promise((resolve, reject) => {
                    ...
                    this.adapter.setForeignState(command.setId, entity.attributes.iobClose, false, {user}, err =>
                        err ? reject(err) : resolve());
                })
            };
            entity.context.COMMANDS.push(command);
        ...
        if (state && state.id) {
            ...
            if (objects[state.id] && objects[state.id].common && objects[state.id].common.write) {
                playPauseCommand = {
                    ...
                    setId: state.id,
                    parseCommand: (entity, command, params, user) => new Promise((resolve, reject) =>
                        this.adapter.setForeignState(command.setId, entity.state !== 'playing', false, {user}, err =>
                            err ? reject(err) : resolve()))
                };
