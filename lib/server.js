...
class WebServer {
    constructor(options) {
        ...
        this._objectData = {
            ...
            usedKeys:            []  //temporary storage for used keys
        };

        this.converter = {
            [Types.socket]:                 processSocket,
            ...
            [Types.motion]:                 processBinarySensors.processMotion,
            [Types.window]:                 processBinarySensors.processWindow,
            [Types.windowTilt]:             processSensors.processWindowTilt,
            [Types.door]:                   processBinarySensors.processDoor,
            [Types.button]:                 processSocket,
            [Types.temperature]:            processSensors.processTemperature,
            [Types.humidity]:               processSensors.processHumidity,
            [Types.lock]:                   processLock,
            [Types.thermostat]:             processThermostat,
        ...
        this.adapter.getForeignObjectAsync('system.config')
            ...
            .then(state => {
                ...
                if (state && (this._themes[state.val] || state.val === 'default')) {
                    ...
                    this._currentThemeDark = this.config.defaultThemeDark || 'default';
                    return this.adapter.setStateAsync(this.adapter.namespace + '.control.themeDark', this._currentThemeDark, true);
                }
            })
            .then(() => {
                // check every minute
    ...
    async _readAllEntities() {
        ...
        for (const dev of smartDevices) {
            const foundIndex = entityData.entities.findIndex(x => x.entity_id === dev.entity_id);
            if (foundIndex !== -1) {
                entityData.entities[foundIndex] = dev;
            } else {
                entityData.entities.push(dev);
            }
        }
    ...
    async _getAllEntities() {
        try {
            ...
            await this._processEntities(ids, entityData.entities);
        } catch (e) {
    ...
    async _processManualEntity(id) {
        try {
            ...
            if (custom.states && custom.states.state && custom.states.state !== id) {
                this.log.error(`Please define custom settings on main object ${custom.states.state} and not on ${id}. Entity skipped`);
                return [];
            }

            if (entityType === 'light') {
                return converterLight.processManualLight(id, obj, entity, this._objectData.objects);
            } else if(entityType === 'input_datetime') {
                return converterDatetime.processManualDateTime(id, obj, entity, this._objectData.objects);
                ...
                return converterAlarmCP.processManualEntity.call(this, id, obj, entity);
                ...
                entity.attributes.assumed_state = custom.assumed_state || true;
                    ...
                    if (entity.context.ATTRIBUTES) {
                        ...
                        for (const attr of attributes) {
                            ...
                            if (attr.getParser) {
                                ...
                                if (entity.context.type === 'light' && typeof(state.val) === 'number'){
                                    // is dimmer
                                    utils.setJsonAttribute(entity.attributes, attr.attribute, this._iobState2EntityState(null,  state.val * 255 / entity.attributes.iob_max, attr.attribute), this.log);
                                } else {
                ...
                for (const control of controls) {
                    if (this.converter[control.type]) {
                        ...
                        if (entities && entities.length) {
                            ...
                            entities.push(processBinarySensors.processBattery.call(this, control, friendlyName, roomName, funcName, objects));
        ...
        if (foundRoom && foundFunc) {
            ...
            if (entities.length) {
                for (const entity of entities) {
                    ...
                    const index = entityData.iobID2entity[id].findIndex(e => e.entity_id === entity.entity_id);
                    if (index === -1) {
                        //this should not happen?? Why was this not added before?
                        this.log.warn(`Object ${id} changed, somehow entity ${entity.entity_id} did not update correctly. Please report.`);
                        entityData.iobID2entity[id].push(entity);
                    } else {
                        entityData.iobID2entity[id][index] = entity; //overwrite old entity.
                    }
                }
        ...
        if (id === this.adapter.namespace + '.configuration') {
            ...
            this._updateTimer = setTimeout(async () => {
                ...
                while (this._objectData.updatedObjects.length > 0) {
                    const {id, obj} = this._objectData.updatedObjects.shift();
                    //this.log.debug(`Start processing of changed object ${id}. Now working on ${promises.length} changes.`);
                    //object deleted:
                    let changed = false; //what do do, when changed? Update all states? How to notify frontend of removed / changed entities?
                    if (!obj) {
                        changed = utils.removeIdFromEntities(id); //handles remove
                        if (changed) {
                            this.log.debug(`Object ${id} deleted, adjusted entities`);
                        } else {
                            this.log.debug(`Object ${id} deleted, not relevant for us.`);
                        }
                    } else {
                        ...
                        if (obj.common && obj.common.custom && obj.common.custom[this.adapter.namespace]) {
                            ...
                            const promise = (async () => {
                                this.log.debug('Blub');
                                await this._processManualEntity(id);
                        ...
                        if (this.adapter.config.aliasOnly) {
                            if (!id.startsWith('alias.0.')) {
                                ...
                                promises.push((async () => {
                                    const result = await this._updateOneDevice(id); //start auto creation for one device.
                                    if (result.length) {
                                        this.log.debug(`Object ${id} changed, required update of auto entity.`);
                                    }
                                })());
                            }
                        } else {
