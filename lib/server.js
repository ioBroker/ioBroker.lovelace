...
class WebServer {
    ...
    _processWindowTilt(id, control, name, room, func, _obj, objects) {
        ...
        if (state && state.id) {
            ...
            entity.context.STATE.getParser = function (entity, attr, state) {
                state = state || {val: null};
                if (_obj.common.states) {
                    entity.state = _obj.common.states[state.val] || 'error';
                } else {
                    //use 0 = closed, 1 = tilted, 2 = open as default, if no common.states
                    entity.state = state.val === 0 ? 'closed' : state.val === 1 ? 'tilted' : 'open';
                }
            };
            this._addID2entity(state.id, entity);
        }
