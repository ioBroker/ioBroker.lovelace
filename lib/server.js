...
class WebServer {
    ...
    _processCommon(id, name, func, room, obj, entityType, entity_id) {
        ...
        const _name = this._generateName(obj, 'en').replace(/[^a-zA-Z0-9А-Яа-яÄÜÖßäöü_]/g, '_');
        ...
        for (let day = 0; day < 5; day++) {
            ...
            if (somethingFound) {
                ...
                if (state && state.id) {
                    ...
                    entity.context.ATTRIBUTES.push({attribute: `forecast.${hassCounter}.datetime`, dayShift: hassCounter});
                }
    ...
    _processLocation(id, control, name, room, func, _obj) {
        ...
        if (state && state.id) {
            entity.context.ATTRIBUTES.push({attribute: 'latitude', gpsCombine: true, getId: state.id});
            this._addID2entity(state.id, entity);
        } else {
    ...
    _processMediaPlayer(id, control, name, room, func, _obj) {
        // - media_player =>
        // STATE on/off/playing/paused/idle/standby/unknown,
        //     attributes: [
        ...
        if (state && state.id) {
            entity.context.ATTRIBUTES.push({command: 'media_play_pause', setId: state.id, setValue: true});
        }

        state = control.states.find(s => s.id && s.name === 'PAUSE');
        if (state && state.id) {
            entity.context.ATTRIBUTES.push({command: 'media_play_pause', setId: state.id, setValue: false});
        } else {
            state = control.states.find(s => s.id && s.name === 'STOP');

            if (state && state.id) {
                entity.context.ATTRIBUTES.push({attribute: 'media_play_pause', setId: state.id, setValue: false});
            }
        }
        entity.context.attributes.supported_features = 0;

        state = control.states.find(s => s.id && s.name === 'NEXT');
        if (state && state.id) {
            entity.context.ATTRIBUTES.push({attribute: 'media_next_track', setId: state.id});
            entity.context.attributes.supported_features |= 0x20;
        }

        state = control.states.find(s => s.id && s.name === 'PREV');
        if (state && state.id) {
            entity.context.ATTRIBUTES.push({attribute: 'media_previous_track', setId: state.id});
            entity.context.attributes.supported_features |= 0x10;
        }

        state = control.states.find(s => s.id && s.name === 'VOLUME');
        let volume_set;
        if (state && state.id) {
            volume_set = {attribute: 'volume_set', setId: state.id};
            entity.context.ATTRIBUTES.push(volume_set);
        }
        ...
        if (state && state.id) {
            ...
            volume_set.getId = volume_set.setId;
            this._addID2entity(volume_set.getId, entity);
        }
        ...
        if (state && state.id) {
            entity.context.ATTRIBUTES.push({attribute: 'turn_on',  setId: state.id, setValue: true});
            entity.context.ATTRIBUTES.push({attribute: 'turn_off', setId: state.id, setValue: false});
            entity.context.ATTRIBUTES.push({attribute: 'power', getId: state.id});
            this._addID2entity(state.id, entity);
        }
        ...
        if (state && state.id) {
            ...
            entity.context.ATTRIBUTES.push({attribute: 'volume_mute', setId: state.id, variable: 'is_volume_muted'});
            this._addID2entity(state.id, entity);
        }
    ...
    _processManualEntity(id, _entities, cb) {
        this.adapter.getForeignObject(id, (err, obj) => {
            ...
            if (entityType === 'light') {
                ...
                entity.context.ATTRIBUTES = [{attribute: 'remaining', getId: id, setId: id}];
            }
    ...
    _processCall(ws, data) {
        ...
        if (data.service === 'toggle') {
            ...
            this.adapter.getForeignState(id, {user}, (err, state) =>
                this.adapter.setForeignState(id, state ? !state.val : true, {user}, () =>
                    this._sendResponse(ws, data.id)));
        } else
        if (data.service === 'volume_set') {
            this.log.debug('volume_set ' + id);

            this.adapter.setForeignState(id, data.service_data.value, {user}, () =>
                this._sendResponse(ws, data.id));
        } else
        if (data.service === 'trigger' || data.service === 'turn_on' || data.service === 'unlock') {
            this.log.debug(data.service + ' ' + id);

            this.adapter.setForeignState(id, true, {user}, () => {
                this._sendResponse(ws, data.id);
                ...
                if (data.service_data.brightness_pct !== undefined) {
                    if (entity.context.ATTRIBUTES) {
                        ...
                        if (attr) {
                            this.adapter.setForeignState(attr.setId, data.service_data.brightness_pct, {user});
                        }
                    }
                }
            });
        ...
        if (data.service === 'turn_off' || data.service === 'lock') {
            ...
            this.adapter.setForeignState(id, false, {user}, () =>
                this._sendResponse(ws, data.id));
        } else
        if (data.service === 'set_temperature') {
            ...
            if (data.service_data.temperature !== undefined) {
                if (entity.context.ATTRIBUTES) {
                    ...
                    if (attr) {
                        return this.adapter.setForeignState(attr.setId, data.service_data.temperature, {user}, () =>
                            this._sendResponse(ws, data.id));
                    }
                }
            }
        ...
        if (data.service.startsWith('set_') && data.service !== 'set_datetime') {
            ...
            this.adapter.setForeignState(id, data.service_data[data.service.substring(4)], {user}, () =>
                this._sendResponse(ws, data.id));
        } else
        if (data.service === 'volume_mute') {
            ...
            this.adapter.setForeignState(id, data.service_data.is_volume_muted, {user}, () =>
                this._sendResponse(ws, data.id));
        } else
        if (data.service.startsWith('select_')) {
            ...
            this.adapter.setForeignState(id, data.service_data[data.service.substring(7)], {user}, () =>
                this._sendResponse(ws, data.id));
        } else
        if (data.service.startsWith('alarm_')) {
            ...
            this.adapter.getForeignObject(id, (err, obj) => {
                if (obj.native.alarm_code && obj.native.alarm_code.toString() !== data.service_data.code.toString()) {
                    ...
                    this.adapter.setForeignState(id, data.service.startsWith('alarm_arm'), {user}, () =>
                        this._sendResponse(ws, data.id));
                }
            });
        } else {
                ...
                this.adapter.getForeignStates(ids, (err, states) => {
                    if (ids && ids.length) {
                        ...
                        ids.forEach((id, i) => {
                            ...
                            if (entity.context.ATTRIBUTES[i].gpsCombine) {
                                let parts = ((states[id] || {}).val || '').toString().split(';');
                                if (parts.length !== 2) {
                                    parts = ((states[id] || {}).val || '').toString().split(',');
                                }
                                if (parts.length === 2) {
                                    entity.attributes.latitude = parseFloat(parts[0].replace(',', '.'));
                                    entity.attributes.longitude = parseFloat(parts[1].replace(',', '.'));
                                } else {
                                    entity.attributes.latitude = 'unknown';
                                    entity.attributes.longitude = 'unknown';
                                }
                            } else
                            if (states[id] && states[id].val !== undefined) {
                                setJsonAttribute(entity.attributes, attribute, this._iobState2EntityState(null, states[id].val, attribute));
                            } else if (entity.context.ATTRIBUTES[i].dayShift !== undefined) {
                                const date = new Date();
                                if (entity.context.ATTRIBUTES[i].dayShift) {
                                    date.setDate(date.getDate() + entity.context.ATTRIBUTES[i].dayShift);
                                }
                                setJsonAttribute(entity.attributes, attribute, date.toISOString());
                            } else {
                                setJsonAttribute(entity.attributes, attribute, 'unknown');
                            }
            ...
            entities.forEach(entity => {
                ...
                if (entity.context.ATTRIBUTES) {
                    ...
                    if (attr) {
                        ...
                        entity.last_updated = new Date(state.ts).toISOString();
                        if (attr.gpsCombine) {
                            let parts = state.val.toString().split(';');
                            if (parts.length !== 2) {
                                parts = state.val.toString().split(',');
                            }
                            if (parts.length === 2) {
                            ...
                            setJsonAttribute(entity.attributes, attr.attribute, this._iobState2EntityState(null, state.val, attr.attribute));
                            if (attr.attribute === 'remaining' && entity.context.type === 'timer') {
                                // - timer => STATE idle/paused/active, attributes: [remaining]
                                if (!state.val) {
                                    entity.state = 'idle';
                                } else
                                if (entity.context.lastValue === state.val) {
                                    // pause
                                    entity.state = 'paused';
                                } else {
                                    // active
                            ...
                            // check dayShift
                            entity.context.ATTRIBUTES.forEach(e => {
                                if (e.dayShift !== undefined) {
                                    const date = new Date();
                                    e.dayShift && date.setDate(date.getDate() + e.dayShift);
                                    setJsonAttribute(entity.attributes, e.attribute, date.toISOString());
                                }
                            });
                        }
                    } else {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (lines[i].trim().match(/<\/body>/)) {
                ...
                if (this.config.hideHeader) {
                    hideScript = 'hideHeader();';
                }
                nLines.push('<script>\n' + fs.readFileSync(__dirname + '/../assets/index.js').toString('utf-8') + hideScript + '\n</script>');
            }
    ...
    _getThemes() {
        ...
        return {
            themes: this._themes,
            default_theme: this.config.defaultTheme
        };
    }
    ...
    _initConnection(ws) {
        ...
        ws.on('message', message => {
            ...
            if (message.type === 'get_panels') {
                ...
                const url = new URL(message.path);
                url.searchParams.set('signed', ws.__auth.access_token);
                this._sendResponse(ws, message.id, url.toString());
            } else
    ...
    destroy(cb) {
        ...
        this.adapter.setState('session', JSON.stringify(this._auth_flows), () => {
