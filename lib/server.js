...
class WebServer {
    constructor(options) {
        ...
        this._notifications = new PersistentNotifications({
            adapter: this.adapter,
            server: this
        });
    ...
    async _processCall(ws, data) {
        ...
        if (this._notifications.processCallService(ws, data)) {
    ...
    async onStateChange(id, state, forceUpdate = false) {
        if (state) {
            ...
            this._notifications.onStateChange(id, state);
        }
    ...
    async _initConnection(ws) {
        ...
        ws.on('message', async message => {
            ...
            if (message.type === 'subscribe_events') {
                ...
                this._sendResponse(ws, message.id, []); //always send empty device list for now.
            } else if (message.type.startsWith('shopping_list/items')) {
            ...
            } else if (message.type.startsWith('logbook/') || ['config/auth/list', 'trace/contexts'].includes(message.type)) {
                await this._logbook.processMessage(ws, message);
            } else if (message.type.startsWith('persistent_notification/')) {
                await this._notifications.processMessage(ws, message);
            } else {
