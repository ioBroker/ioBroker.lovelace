...
class WebServer {
        ...
        this._app.get('/state/*', async (req, res) => {
            try {
                ...
                try {
                    ...
                    // connection to client not open anymore. Don't kill server here!
                    this.log.debug('Connection to client already colsed?: ' + innerE + ' coult not send error ' + e);
                }
            }
        });
    ...
    async _initConnection(ws) {
        ...
        if (this.config.auth) {
            //tell homeassistant to authorize itself.
