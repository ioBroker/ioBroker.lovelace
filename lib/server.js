...
class WebServer {
    ...
    processCommon(id, name, func, room, obj, entityType) {
        if (!name) {
            if (func && room) {
                name = room + ' ' + func;
            } else {
        ...
        const _name = this._generateName(obj, 'en');
        const entity = {
        ...
        this._entity2ID[entity.entity_id] = obj;
        this._entities.push(entity);
        // entity.state = this._iobState2EntityState(obj._id, state.val);
        return entity;
    }

    processSocket(id, control, name, room, func, _obj) {
        const entity = this.processCommon(id, name, room, func, _obj, 'switch');
    ...
    processLight(id, control, name, room, func, _obj) {
        const entity = this.processCommon(id, name, room, func, _obj, 'light');
    ...
    processDimmer(id, control, name, room, func, _obj) {
        const entity = this.processCommon(id, name, room, func, _obj, 'light');
    ...
    _processEntity(id, _entities, cb) {
        this.adapter.getForeignObject(id, (err, obj) => {
            ...
            const entity = this.processCommon(id, name, null, null, obj, entityType);
    ...
    onStateChange(id, state) {
        ...
        if (entity) {
            ...
            if (entity.context.STATE.getId === id) {
                ...
                const attr = entity.context.ATTRIBUTES.find(e = b<)
            }
    ...
    _manageSubscribesFromConfig() {
        ...
        const newIds = ids.filter(id => this.subscribed.indexOf(id) === -1 && this._entity2ID[id]);
        const deleted = this.subscribed.filter(id => ids.indexOf(id) === -1 && this._entity2ID[id]);
        deleted.forEach(id => this.adapter.unsubscribeForeignStates(this._entity2ID[id]._id));
        newIds.forEach(id => this.adapter.subscribeForeignStates(this._entity2ID[id]._id));
    }
    ...
    _init() {
        ...
        this.app.get('api/history/period/*', (req, res) => {
            res.json({});
        });
        ...
        if (data.service === 'toggle') {
            ...
            this.adapter.setForeignState(id, true, err => {
                if (data.service_data.brightness_pct !== undefined) {
                    this.log.debug('brightness is not yet supported! ' + id + ' = ' + data.service_data.brightness_pct);
                    this._sendResponse(ws, data.id);
                } else {
                    this._sendResponse(ws, data.id);
                }
