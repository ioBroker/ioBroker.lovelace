...
class WebServer {
    ...
    _iobState2EntityState(entity, val, type) {
    ...
    async _processManualEntity(id, forcedEntityId) {
        try {
            ...
            if (entityType === 'light') {
                ...
                const state = await this.adapter.getForeignStateAsync(id);
                entity.attributes.initial = this._iobState2EntityState(entity, state ? state.val : undefined);
            }
                ...
                if (state) {
                    ...
                    if (entity.context.STATE.getId === id) {
                        ...
                        if (entity.context.STATE.getParser) {
                            ...
                            entity.state = this._iobState2EntityState(entity, state.val);
                        }
                    }
                    ...
                    if (entity.context.ATTRIBUTES) {
                        ...
                        for (const attr of attributes) {
                            ...
                            if (attr.getParser) {
                                ...
                                utils.setJsonAttribute(entity.attributes, attr.attribute, this._iobState2EntityState(entity, state.val, attr.attribute), this.log);
                            }
                        }
                    }
                }
                                ...
                                if (e.val !== null) {
                                    ...
                                    const result = {
                                        ...
                                        state: typeof entity.context.STATE.historyParser === 'function' ?
                                            ...
                                            this._iobState2EntityState(entity, e.val).toString(),
                                        last_changed: ts,
                                        last_updated: ts,
    ...
    async _processBrowserModMessage(message) {
        ...
        if (method === 'update') {
            ...
            await this.adapter.setObjectNotExistsAsync(id, { type: 'device', common: { name: message.deviceID }, native: {instance: message.deviceID } });
            if (message.data && message.data.browser) {
                if (message.data.browser.path) {
                    await this.adapter.setObjectNotExistsAsync(id + '.path', {type: 'state', common: {
                        name: 'UI Path',
                        type: 'string',
                        read: true,
                        write: true,
                    ...
                    await this.adapter.setStateAsync(id + '.path', message.data.browser.path, true);
                }
                if (message.data.browser.visibility) {
                    await this.adapter.setObjectNotExistsAsync(id + '.visible', {type: 'state', common: {
                        name: 'UI Visible',
                        type: 'boolean',
                        read: true,
                        write: false,
                    ...
                    await this.adapter.setStateAsync(id + '.visible', message.data.browser.visibility === 'visible', true);
                }
                if (message.data.browser.battery_level) {
                    await this.adapter.setObjectNotExistsAsync(id + '.battery', {type: 'state', common:
                        {
                            name: 'battery',
                            type: 'number',
                            read: true,
                    ...
                    await this.adapter.setStateAsync(id + '.visible', message.data.browser.battery_level, true);
                }
            }
        } else {
    ...
    async _initConnection(ws) {
        ...
        ws.on('message', async message => {
            ...
            if (message.type.startsWith('browser_mod')) {
                this._processBrowserModMessage(message);
            } else {
