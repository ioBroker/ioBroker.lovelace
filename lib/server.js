...
class WebServer {
     ...
     * @param roomName room name assigned to object
     * @param funcName func name assigned to object
     * @param result array of created entities if any
     * @param forcedEntityId possible forced entity_id on conflict cases.
     * @returns {Promise<void>} resolves when done.
     */
                    ...
                    if (this.converter[control.type]) {
                        ...
                        if (entities && entities.length) {
                            ...
                            if (mainEntity) {
                                ...
                                entities.push(
                                    converterBinarySensors.processBattery(
                                        control,
                                        friendlyName,
                                        roomName,
                                        funcName,
                                ...
                                entities.push(
                                    converterBinarySensors.connectivityIndicator(
                                        control,
                                        friendlyName,
                                        roomName,
                                        funcName,
                                ...
                                entities.push(
                                    converterBinarySensors.processError(
                                        control,
                                        friendlyName,
                                        roomName,
                                        funcName,
                                ...
                                entities.push(
                                    converterBinarySensors.processMaintenance(
                                        control,
                                        friendlyName,
                                        roomName,
                                        funcName,
                                ...
                                entities.push(
                                    converterBinarySensors.processWorking(
                                        control,
                                        friendlyName,
                                        roomName,
                                        funcName,
                ...
                for (const id of func.common.members) {
                    for (const room of Object.values(this._objectData.rooms)) {
                        ...
                        if (!this._objectData.roomNames[room._id]) {
                            // Get the name of room (with language and if name is empty)
                            this._objectData.roomNames[room._id] = utils.getEnumName(room, this.lang);
                        }
                        ...
                        if (pos !== -1) {
                            await this._processIobState(
                                ...
                                this._objectData.objects,
                                id,
                                this._objectData.roomNames[room._id],
                                funcName,
                                result,
                            );
                        }
    ...
    _init() {
        ...
        this._app.get('/adapter/*', async (req, res) => {
    ...
    async _replyWithImage(req, res){
        this.log.debug(`Get image for ${req.params?.entity_id} access_token=${req.query?.signed || req.query?.token}`);
        try {
            const data = await this._getImage(
                req.params?.entity_id,
                req.query?.token || 'empty',
                req.query?.signed || req.query?.token,
                req.url,
            );
        ...
        if (url) {
            ...
            if (image) {
                if (image.file === null || image.file === undefined) {
                    ...
                    image.mimeType =
                        image.mimeType ||
                        (mime.getType || mime.lookup).call(image.mimeType, url) ||
                        'text/javascript';
