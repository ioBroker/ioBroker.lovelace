...
class WebServer {
                ...
                if (attr.is_rgb_string || attr.is_rgb_values) {
                    ...
                    this.adapter.setForeignState(attr.setId, h / 360 * attr.max, false, {user}, err => {
                        if (err) {
                            ...
                            if (attr_Sat) {
                                this.adapter.setForeignState(attr_Sat.setId, s / 100 * attr.max, false, {user}, err => err ? reject(err) : resolve());
                            } else {
    ...
    _lightAdvancedAddHueAndSat(control, objects, entity) {
        ...
        if (sat && sat.id) {
            ...
            this.log.debug('Hue entity: ' + JSON.stringify(entity, null, 2));
