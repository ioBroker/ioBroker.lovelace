...
const handleAutoEntitiesCard = require('../build/lib/modules/autoEntities').handleAutoEntitiesCard;
const utils = require('../build/lib/entities/utils');
const BrowserModModule = require('../build/lib/modules/browser_mod');
const entityData = require('./dataSingleton');
const bindings = require('./bindings');
const getFriendlyName = require('./entities/friendly_name').getFriendlyName;
const iobState2EntityState = require('../build/lib/converters/genericConverter').iobState2EntityState;
const NUMERIC_DEVICE_CLASSES = require('../build/lib/converters/genericConverter').numericDeviceClasses;
const HistoryModule = require('../build/lib/modules/history');
const ConversationModule = require('../build/lib/modules/conversation');
const LogbookModule = require('../build/lib/modules/logbook');
const PersistentNotifications = require('../build/lib/modules/persistentNotifications');
const TodoModule = require('../build/lib/modules/todo');
const PersonModule = require('../build/lib/modules/person');
const StatisticsRecorderModule = require('../build/lib/modules/statisticsRecorder');
const EntityRegistry = require('../build/lib/modules/entityRegistry');
const DashboardModule = require('../build/lib/modules/dashboard');
const DeviceRegistryModule = require('../build/lib/modules/deviceRegistry');
const AreaRegistryModule = require('../build/lib/modules/areaRegistry');
...
const { Converter } = require('../build/lib/converters/converter');
const ignoreIds = [/^system\./, /^script\./];

const converterSwitch = require('../build/lib/converters/switch');
...
const converterBinarySensors = require('../build/lib/converters/binary_sensor');
const converterSensors = require('../build/lib/converters/sensor');
const converterGeoLocation = require('../build/lib/converters/geo_location');
const converterDatetime = require('../build/lib/converters/input_datetime');
const converterAlarmCP = require('../build/lib/converters/alarm_control_panel');
const converterInputSelect = require('../build/lib/converters/input_select');
const convertFan = require('../build/lib/converters/fan');
const converterClimate = require('../build/lib/converters/climate');
const converterLight = require('../build/lib/converters/light');
// eslint-disable-next-line @typescript-eslint/no-unused-vars
const _registeredConverters = {
    converterLock: require('../build/lib/converters/lock'),
    converterCamera: require('../build/lib/converters/camera'),
    converterWeather: require('../build/lib/converters/weather'),
    converterCover: require('../build/lib/converters/cover'),
    converterMediaPlayer: require('../build/lib/converters/media_player'),
};
        ...
        try {
            ...
            for (const id of ids) {
                ...
                for (const entity of entities) {
                    // Remove any auto-detected entities that share state IDs with this manual entity
                    // so that manual entities always win over auto-detected ones for the same ioBroker ID.
                    for (const stateId of entity.context.ids || []) {
                        for (const existing of [...(entityData.iobID2entity[stateId] || [])]) {
                            if (!existing.isManual) {
                                utils.removeEntity(existing);
                            }
                        }
                    }
                    created.push(entity);
    ...
    async _processIobState(ids, objects, id, room, func, existingEntities) {
        ...
        try {
            ...
            if (controls) {
                ...
                await Converter.convertAll(controls, {
                    id,
                    friendlyName,
                    room,
                    func,
    ...
    async _updateById(id, idsAutomaticallyProcessed, entitiesNeedsUpdate) {
        ...
        if (!obj) {
            ...
            if (obj.common && obj.common.custom && obj.common.custom[this.adapter.namespace]) {
                ...
                const newEntities = await this._processManualEntity(id);
