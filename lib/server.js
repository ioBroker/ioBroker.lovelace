...
class WebServer {
    constructor(options) {
        ...
        this._createObjectsFromArrayOfIds = utils.createObjectsFromArrayOfIds.bind(this);
        this._removeIdFromEntities = utils._removeIdFromEntities.bind(this); //removes an entity from our caches.
    ...
    async _processManualEntity(id) {
        try {
            ...
            if (custom.states && custom.states.state && custom.states.state !== id) {
                this.adapter.error(`Please define custom settings on main object ${custom.states.state} and not on ${id}. Entity skipped`);
                return [];
            }

            if (entityType === 'light') {
                return converterLight.processManualLight.call(this, id, obj, entity);
            } else if(entityType === 'input_datetime') {
                return converterDatetime.processManualDateTime.call(this,id, obj, entity);
