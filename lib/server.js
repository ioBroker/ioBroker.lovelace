...
class WebServer {
    constructor(options) {
        ...
        //initialize modules.
        this._browserMod = new BrowserModModule({
            adapter: this.adapter,
            objects: this._objectData.objects
        });
        this._history = new HistoryModule({
            adapter: this.adapter,
            entityData: entityData
        });
        this._conversation = new ConversationModule({
            adapter: this.adapter,
            sendResponse: this._sendResponse,
            lang: this.lang,
            words: this.words
        });
        this._logbook = new LogbookModule({
            adapter: this.adapter,
            getUsedEntityIDs: () => {
                const entities = [];
        ...
        const concurrentPromises = [
            this.adapter.getForeignObjectAsync('system.config')
                ...
                }).then(() => this._browserMod.init(this._lovelaceConfig)),
    ...
    async _processSingleCall(ws, data, entity_id) {
        ...
        if (data.service === 'trigger' || data.service === 'turn_on' || data.service === 'unlock' || data.service === 'press') {
    ...
    async onStateChange(id, state, forceUpdate = false) {
        if (state) {
            ...
            this._logbook.onStateChange(id, state, this._wss);
            this._conversation.onStateChange(id, state, this._wss);
        }
        ...
        await this._browserMod.onStateChange(id, state);
    }
        ...
        try {
            ...
            for (const file of staticCards) {
                ...
                this._ressourceConfig.push({
                    type: 'module',
                    url: '/lovelace/static_cards/' + file
                });
            }
    ...
    async _manageSubscribesFromConfig() {
        ...
        if (this._lovelaceConfig.views) {
            ...
            promises.push(this._browserMod.handeUpdatedConfig(this._lovelaceConfig));
        }
    ...
    _getConfig() {
        ...
        const configObj = {
            ...
            components: [ //TODO: experiment with notify.html5.
                'lovelace', 'conversation', 'shopping_list', 'energy', 'notify.html5', 'logbook'
            ],
            version: VERSION,
            state: 'RUNNING',
    ...
    _init() {
        ...
        this._app.get('/api/history/period/:start', async (req, res) => {
            this._history.processRequest(req, res);
        });
    ...
    _getShortEntity(entity) {
        return {
            s: entity.state,
            a: entity.attributes,
            lc: entity.last_changed && typeof entity.last_changed.getTime === 'function' ? entity.last_changed.getTime() / 1000 : Date.now() / 1000,
            lu: entity.last_updated && typeof entity.last_updated.getTime === 'function' ? entity.last_updated.getTime() / 1000 : undefined
        };
    }
        ...
        ws.on('message', async message => {
            ...
            if (message.type === 'subscribe_entities') {
                ...
                if (ws._subscribes) {
                    ...
                    ws._subscribes.subscribe_entities.push(Number(message.id));
                }
                //this._sendResponse(ws, message.id);
            ...
            if (message.type === 'render_template') {
                ...
                await this._history.processMessage(ws, message);
            } else if (message.type.startsWith('assist_pipeline/') || message.type.startsWith('conversation/')) {
                await this._conversation.processMessage(ws, message);
            } else if (message.type.startsWith('logbook/') || ['config/auth/list', 'trace/context'].includes(message.type)) {
                await this._logbook.processMessage(ws, message);
            } else {
