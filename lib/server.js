...
class WebServer {
    ...
    _processMediaPlayer(id, control, name, room, func, _obj) {
        ...
        if (state && state.id) {
            entity.context.ATTRIBUTES.push({command: 'media_play_pause', setId: state.id, value: true});
        }

        state = control.states.find(s => s.id && s.name === 'PAUSE');
        if (state && state.id) {
            entity.context.ATTRIBUTES.push({command: 'media_play_pause', setId: state.id, value: false});
        } else {
            state = control.states.find(s => s.id && s.name === 'STOP');

            if (state && state.id) {
                entity.context.ATTRIBUTES.push({attribute: 'media_play_pause', setId: state.id, value: false});
            }
        }
        ...
        if (state && state.id) {
            entity.context.ATTRIBUTES.push({attribute: 'turn_on',  setId: state.id, value: true});
            entity.context.ATTRIBUTES.push({attribute: 'turn_off', setId: state.id, value: false});
        ...
        if (data.service.startsWith('alarm_')) {
            ...
            this.adapter.getForeignObject(id, (err, obj) => {
                if (obj.native.alarm_code && obj.native.alarm_code.toString() !== data.service_data.code.toString()) {
                    ...
                    this.adapter.setForeignState(id, data.service.startsWith('alarm_arm'), () =>
                        this._sendResponse(ws, data.id));
                }
            });
        } else {
    ...
    _init() {
        ...
        this._app.get('/cards/*', (req, res) => {
            ...
            this.adapter.readFile(this.adapter.namespace, file, (err, data) => {
                if (data) {
                    const pos = file.lastIndexOf('.');
                    res.setHeader('content-type', mime.getType(file.substring(pos + 1).toLowerCase()));
                    res.send(data);
                } else {
        ...
        this._app.get('/local/custom_ui/*', (req, res) => {
            ...
            this.adapter.readFile(this.adapter.namespace, file.replace('/local/custom_ui/', '/cards/'), (err, data) => {
                if (data) {
    ...
    _sendShoppingListUpdate() {
        this._wss.clients.forEach(ws => {
            if (ws._subscribes && ws._subscribes.shopping_list_updated) {
                const t = {
                    type: 'event',
                    event: {
                ...
                ws._subscribes.shopping_list_updated.forEach(id => {
                    t.id = id;
                    ws.send(JSON.stringify(t));
                });
            }
