...
class WebServer {
    ...
    async _getCurrentUser(ws) {
        let user = this.adapter.usernames ? this.adapter.usernames[(ws.__auth.username || this.config.defaultUser)] : '';
        if (user) {
            user = user.id;
        } else {
            user = 'system.user.' + (ws.__auth.username || this.config.defaultUser).toLowerCase(); //this is not correct in js-controller 3.2
        }
