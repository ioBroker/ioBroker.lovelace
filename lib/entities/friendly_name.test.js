...
const { getFriendlyName } = require('./friendly_name');
...
describe('entities/friendly_name', function () {
    ...
    it('predefined name wins over everything else', function () {
        const obj = { common: { name: 'ignored', smartName: 'also ignored' }, _id: 'ignored' };
    ...
    it('smartName (string) is used when no predefined name', function () {
        const obj = { common: { name: 'ignored', smartName: 'Smart' }, _id: 'ignored' };
        expect(getFriendlyName(null, obj, 'room', 'func')).to.equal('Smart');
    });

    it('smartName (object) uses configured language', function () {
        entityData.lang = 'de';
        const obj = { common: { name: 'ignored', smartName: { en: 'English', de: 'Deutsch' } }, _id: 'ignored' };
    ...
    it('returns null/falsy when object itself is absent', function () {
        // All candidates return falsy when obj is null → reduce returns null
        expect(getFriendlyName(null, null, null, null)).to.be.null;
    });
});
