...
function updateTimestamps(entity, state, newTS = false) {
    ...
    if (lc / 1000 > entity[lcToCheck] || isNaN(entity[lcToCheck]) || new Date(entity[lcToCheck] * 1000).toString() === 'Invalid Date') {
        entity[lcToCheck] = lc / 1000;
    }
    if (lu / 1000 > entity[luToCheck] || isNaN(entity[luToCheck]) || new Date(entity[luToCheck] * 1000).toString() === 'Invalid Date') {
