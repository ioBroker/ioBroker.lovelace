 ...
 *
 * @param id of object to find parents for
 * @param objects {id: ioBroker.Object} objects cache
 * @returns {string[]} array of parent IDs, starting with direct parent, up to device. If object is device, will return array with only the id itself.
 */
...
function getEnumName(obj, lang, force = false) {
    ...
    if (obj._id && cache[obj._id] && !force) {
 ...
 *
 * @param {string|null|undefined} name friendly name of entity, if empty, will be read from object or generated from room & func or id.
 * @param {object|null|undefined} room room object of device for name generation
 * @param {object|null|undefined} func function object of device for name generation
 ...
 * @returns {{state: string, last_changed: number, last_updated: number, context: {id: string, type: string}, attributes: {friendly_name: string | undefined}, entity_id: string}} new entity object
 */
function processCommon(name, room, func, obj, entityType, entity_id) {
    const entity = {
        entity_id: getEntityId(entityType, entity_id, obj),
        plattform: entityType,
        attributes: {
 ...
 * @param {Record<string, ioBroker.Object>} [objects] optional objects cache to determine writeability of attributes.
 */
function fillEntityFromStates(states, entity, objects) {
    ...
    // can't rely on state, since it might not be present (like in geo_location, has no object for state)
                ...
                if (key !== 'state' && key !== 'stateRead') {
                    ...
                    if (!attr) {
                        entity.context.ATTRIBUTES.push({
                            ...
                            setId: obj?.common?.write ? id : undefined,
                        });
                    } else {
                        attr.setId = id;
                    }
