...
class HistoryModule {
    ...
    async processMessage(ws, message) {
        if (message.type && message.type.startsWith('history/')) {
            ...
            if (message.type === 'history/stream') {
                ...
                ws._subscribes.history.push(message.id);
            ...
            this.subcribeIdsToParameters[message.id] = parameters;
    ...
    onStateChange(id, state, websocketServer) {
        if (state) {
            ...
            for (const [msgId, parameters] of Object.entries(this.subcribeIdsToParameters)) {
                let active = false;
                if (websocketServer) {
                    for (const client of websocketServer.clients) {
                        if (client._subscribes.history && client.readyState === WebSocket.OPEN) {
                            if (client._subscribes.history.includes(Number(msgId))) {
                                active = true;
                                const states = {};
                                ...
                                for (const entity of entityIdsAffected) {
                                    if (parameters.entityIds.includes(entity.entity_id)) {
                                        this.adapter.log.debug('History subscription ' + msgId + ' is for right entity, sending update.');
                                ...
                                if (Object.keys(states).length > 0) {
                                    sendHistoryResponse(client, msgId, states, parameters);
                                }
                            }
                        }
                    }
                ...
                if (!active) {
                    this.adapter.log.debug('History subscription ' + msgId + ' deleted.');
                    delete this.subcribeIdsToParameters[msgId];
                }
            }
        }
    }
