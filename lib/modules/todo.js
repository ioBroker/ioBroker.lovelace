...
class TodoModule {
    ...
    async processMessage(ws, message) {
        if (message.type && (message.type.startsWith('todo/') || message.type.startsWith('shopping_list/'))) {
            ...
            if (message.type === 'todo/item/subscribe') {
                ...
                this._publishUpdate(ws, todoList);
    ...
    async processServiceCall(ws, data) {
        ...
        if (data.domain === 'todo') {
            if (data.service === 'add_item') {
                ...
                if (entity) {
                    const todoList = await this._getTodoList(entity);
                    const item = data.service_data.item;
                    item.status = item.status || TodoItemStatus.NeedsAction;
                    item.uid = item.uid || Math.floor(Math.random() * Date.now());
                    item.due = item.due || null;
                    item.description = item.description || null;
                    todoList.items.push(data.service_data.item);
    ...
    async init() {
        ...
        if (!entityShoppingList) {
            const obj = await this.adapter.getObjectAsync('control.shopping_list');
            entityShoppingList = processCommon('Shopping List', null, null, obj, 'todo', 'todo.shoppinglist');
            entityShoppingList.attributes.icon = 'mdi:cart';
            entityShoppingList.context.STATE = {
                getId: entityShoppingList.context.id,
                setId: entityShoppingList.context.id
            };
        ...
        const todoList = await this._getTodoList(entityShoppingList);
        entityShoppingList.state = String(todoList.items.length);
    }
