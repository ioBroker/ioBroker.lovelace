...
const {updateTimestamps} = require('../entities/utils');
...
class TodoModule {
    ...
    async processMessage(ws, message) {
        if (message.type && message.type.startsWith('todo/')) {

            if (message.type === 'todd/item/subscribe') {
    ...
    async init() {
        ...
        if (!entityShoppingList) {
            entityShoppingList = {
                ...
                'context': {
                    'id': this.adapter.namespace + '.control.shopping_list',
                    'STATE': {
                        getId: this.adapter.namespace + '.control.shopping_list',
                        setId: this.adapter.namespace + '.control.shopping_list'
                    },
                    'type': 'todo'
                }
            };
