...
class EntityRegistry {
    ...
    constructor(options) {
        ...
        //TODO: clean up entities for deleted ioBroker objects from registry on restart -> will keep them during runtime to prevent issue, but we should clean up some time.
    }
     ...
     * @returns {Promise<{}>} entity registry entry.
     */
    ...
    updateEntityFromRegistry(entity, entry) {
        if (entity.isManual) {
            return; //manual entities are not updated from registry.
            //TODO: think about updating entity registry and overwriting automatic entity from manual entity?
        }
        if (!entry) {
            entry = this._entries[entity.context.id];
            if (!entry) {
                //this.adapter.log.debug(`Entity ${entity.context.id} not found in registry`);
                return;
            }
        }
    ...
    async processMessage(ws, message) {
        ...
        //for now handle device registry here, too.
        //need to also send `device_registry_updated` event.
        return false;
    }
