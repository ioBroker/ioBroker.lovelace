...
class LogbookModule {
    constructor(options) {
        ...
        this.instances = [];
        this.adapter.getObjectView('system', 'instance', { startkey: 'system.adapter.', endkey: 'system.adapter.\u9999' }, (err, instances) => {
            if (err) {
                ...
                for (const row of instances.rows) {
                    if (row.value && row.value._id) {
                        ...
                        const name = id.split('.').splice(2).join('.');
                        this.instances.push({
                            name,
                            id
                        });
    ...
    sendLogbookResponse(ws, id, startTime = undefined, endTime = undefined, results = [], partial = false) {
        ...
        for (const result of results) {
            message.event.events.push({
                ...
                context_user_id: result.state.from
            });
        }
        //sort by time:
    ...
    async processMessage(ws, message) {
        ...
        } else if (message.type === 'trace/context') {
            ws.send(JSON.stringify({id: message.id, type: 'result', success: true, result: null})); //say that subscription was successfull.
        }
    }
