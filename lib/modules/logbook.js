        ...
        if (message.type === 'logbook/event_stream') {
            ...
            try {
                ...
                const options = {
                    ...
                    start: startTime,
                    end: Math.min(Date.now(), endTime), //prevent entries in one year from influx
            ...
            ws.send(JSON.stringify({ id: message.id, type: 'result', success: true, result: {} })); //say that trace is empty.
            return true;
        }
        //not processed:
        return false;
