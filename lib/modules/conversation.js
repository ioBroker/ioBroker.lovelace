...
class ConversationModule {
    ...
    async processMessage(ws, message) {
        ...
        if (message.type === 'assist_pipeline/run') {
            ...
            const conversation_id = message.conversation_id || Date.now();
            ...
            ws.__conversations[conversation_id] = {
                ...
                }, this._timeout, conversation_id, message.id),
            };
    ...
    async onStateChange(id, state, websocketServer) {
        if (state) {
            if (id === this.adapter.namespace + '.conversation') {
                if (state.ack) {
                    // send answer to conversation dialog
                    websocketServer && websocketServer.clients.forEach(client => {
                        if (client.__conversations && client.readyState === WebSocket.OPEN) {
                            ...
                            Object.keys(client.__conversations).forEach(conversation_id => {
                                ...
                                this.sendSpeechResponse(state.val, client, client.__conversations[conversation_id].id, conversation_id);
                            });
                        }
                    });
                }
