...
class BrowserModModule {
    ...
    async _checkObjects(ioBrokerDeviceId, browserId) {
        ...
        if (!this.objects[ioBrokerDeviceId + '.name'] && browserId) {
        ...
        if (!this.objects[ioBrokerDeviceId + '.notification']) {
            await this.adapter.setObjectNotExistsAsync(ioBrokerDeviceId + '.notification', {
                type: 'state', common: {
                    name: 'Show notification. Optional as json with fields: message, duration(ms), action_button_text, action',
                    type: 'string',
                    read: false,
                    write: true,
                    role: 'json'
        ...
        if (this.objects[ioBrokerDeviceId + '.toast']) {
            await this.adapter.delObjectAsync(ioBrokerDeviceId + '.toast');
        }
