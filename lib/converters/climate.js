        ...
        if (state && state.id) {
            switch(state.name) {
                ...
                case 'SWING':
                    ...
                    break;
                default:
                    adapterData.log.info(`Unknown state ${states.name} while creating climate entity for ${id}. Please report.`);
            }
        }
    }
...
function fillClimateEntityFromStates(states, objects, entity, iobType) {
    ...
    //add set_hvad_mode command -> used by lovelace to control mode & on/off.
    if (states.state || states.hvac_mode) {
        entity.context.COMMANDS.push({
            ...
            parseCommand: async (entity, command, data, user) => {
                ...
                if (hvac_attr) {
                    value = hvac_attr.lovelaceToIob[value] || value;
    ...
    if (states.state || states.stateRead) {
        ...
        entity.context.STATE.getParser = function (entity, attr, state) {
            ...
            entity.state = state.val ? iobType === typeDetector.Types.airCondition ? 'cool' : 'heat' : 'off';
            if (states.hvac_mode) {
                if (!state.val || entity.attributes.hvac_mode === 'off') {
                    entity.attributes.hvac_mode = entity.state; //overwrite mode only if is 'off' or should be set to 'off'.
                }
            } else {
    ...
    if (states.hvac_mode) {
        ...
        hvac_attr.getParser = function (entity, attr, state) {
            state = state || {val: null};
            entity.attributes.hvac_mode = attr.iobToLovelace[state.val] || 'unknown';
        ...
        hvac_attr.historyParser = (id, val) => {
            return hvac_attr.iobToLovelace[val];
        };
    }
    ...
    if (states.swing_mode) {
        ...
        if (swing_obj && swing_obj.common && swing_obj.common.states) {
            ...
            swing_attr.states = {}; //is we don't know a translation, prevent errors later on.
            entity.attributes.swing_modes = [0, 1, 2, 3, 4, 5, 6, 7]; //add some modes to dropdown...
        }
        swing_attr.getParser = (entity, attr, state) => {
            const val = state ? state.val : null;
            entity.attributes.swing_mode = attr.states[val] || val;
        };
        swing_attr.historyParser = (id, val) => swing_attr.states[val] || val;
        entity.context.COMMANDS.push({
            service: 'set_swing_mode',
            parseCommand: async (entity, command, data, user) => {
                const mode = data.swing_mode;
                let val = mode;
                for (const value of Object.values(swing_attr.states)) {
                    if (value === mode) {
                        val = value;
                    }
                }
    ...
    if (states.fan_mode) {
        ...
        if (fan_obj && fan_obj.common && fan_obj.common.states) {
            ...
            fan_attr.states = {}; //is we don't know a translation, prevent errors later on.
            entity.attributes.fan_modes = [0, 1, 2, 3, 4, 5, 6, 7]; //add some modes to dropdown...
        }
        fan_attr.getParser = (entity, attr, state) => {
            const val = state ? state.val : null;
            entity.attributes.fan_mode = attr.states[val] || val;
        };
        fan_attr.historyParser = (id, val) => fan_attr.states[val] || val;
        entity.context.COMMANDS.push({
            service: 'set_fan_mode',
            parseCommand: async (entity, command, data, user) => {
                const mode = data.fan_mode;
                let val = mode;
                for (const value of Object.values(fan_attr.states)) {
                    if (value === mode) {
                        val = value;
                    }
                }
