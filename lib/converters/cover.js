...
function addBlindLevel(entities, control, objects, name, id, room, func, _obj, forcedEntityId) {
    ...
    if (state && state.id) {
        ...
        if (state && state.id) {
            ...
            const entitySlider = utils.processCommon(
                ...
                parseCommand: (entity, command, data, user) => {
                    return new Promise((resolve, reject) => {
                        ...
                        if (entity.state === 'open' || entity.state === 'opening') {
                            if (down) {
                                adapterData.adapter.setForeignState(down.setId, true, false, { user }, err =>
                                    err ? reject(err) : resolve(),
                                );
                            } else {
                                adapterData.adapter.setForeignState(
                                    command.setId,
                                    entity.context.STATE.invert ? entity.context.STATE.min : entity.context.STATE.max,
                                    false,
                                    { user },
                            ...
                            if (up) {
                                adapterData.adapter.setForeignState(up.setId, true, false, { user }, err =>
                                    err ? reject(err) : resolve(),
                                );
                            } else {
                                adapterData.adapter.setForeignState(
                                    command.setId,
                                    entity.context.STATE.invert ? entity.context.STATE.max : entity.context.STATE.min,
                                    false,
                                    { user },
                                    err => (err ? reject(err) : resolve()),
                                );
