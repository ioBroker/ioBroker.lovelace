...
function timeToAttributes(entity, date) {
    ...
    if (entity.attributes.has_date) {
        ...
        entity.state = `${entity.attributes.year}-${entity.attributes.month}-${entity.attributes.day}`;
    }
    if (entity.attributes.has_time) {
        entity.attributes.hour = (date.getHours() < 10 ? '0' : '') + date.getHours();
        entity.attributes.minute = (date.getMinutes() < 10 ? '0' : '') + date.getMinutes();
        entity.attributes.second = (date.getSeconds() < 10 ? '0' : '') + date.getSeconds();
        if (!entity.attributes.has_date) {
            entity.state = '';
        } else {
            entity.state += ' ';
