...
exports.processManualEntity = async function (id, obj, entity, objects, custom) {
    ...
    return fillAlarmControlPanelFromStates.call(this, states, objects, entity);
};

/**
 * Augment the entity from the functions found / defined.
 *
 * @param states {Record<string, string>} - translation from function to ioBroker.Object ids.
 * @param objects {Record<string, ioBroker.Object>} - id object cache.
 * @param entity {object} - already created entity.
 * @returns {*[]} - created entity in array.
 */
function fillAlarmControlPanelFromStates(states, objects, entity) {
    ...
    // fill default states -> attributes.
    utils.fillEntityFromStates(states, entity, objects);
        ...
        if (obj && obj.common) {
            if (obj.common.custom && obj.common.custom[adapterData.adapter.namespace]) {
                entity.attributes.code_format =
                    obj.common.custom[adapterData.adapter.namespace].code_format || 'number';
            }
    ...
    const processCommand = async (entity, command, data, user) => {
        ...
        if (!adapterData.adapter.config.alarmCheckCodeOnDisarmOnly || targetState.includes('disarm')) {
            ...
            if (!obj.native.alarm_code) {
                adapterData.log.warn(
                    `No code is defined! To provide the code add to object ${id} native.alarm_code with desired code`,
                );
        ...
        if (entity.context.STATE.isBoolean) {
            ...
            if (entity.context.STATE.map) {
                valToSet = Number(
                    Object.keys(entity.context.STATE.map).find(key => entity.context.STATE.map[key] === targetState),
                );
...
/**
 * Augment services with alarm_control_panel.
 *
 * @param services {Record<string, any>} - services to augment.
 */
