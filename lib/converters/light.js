...
function _lightAdvancedAddRGBSingle(control, objects, entity) {
...
function _setLightAdvancedAttributesToIOBStates(data, entity, user, resolve, reject) {
...
function _parseLightAdvancedOn(entity, command, data, user) {
            ...
            this.adapter.getForeignStates(ids, {user}, (err, state) => {
                ...
                if (!state[entity.context.STATE.getId] || !state[entity.context.STATE.getId].val) {
                    ...
                    this.adapter.setForeignState(entity.context.STATE.setId, command.on, false, {user}, () =>
                        _setLightAdvancedAttributesToIOBStates.call(this, data, entity, user, resolve, reject));
                } else {
                    _setLightAdvancedAttributesToIOBStates.call(this, data, entity, user,resolve, reject);
                }
            });
...
exports.processLightAdvanced = function (id, control, name, room, func, _obj, objects) {
    ...
    if (state && state.id) {
        ...
        _lightAdvancedAddState.call(this, control,state, entity);

        //fill in color temperature stuff.
        _lightAdvancedAddColorTemperature.call(this, control, objects, entity);

        //if there is a "BRIGHTNESS" control, use it to dim lamp.
        _lightAdvancedAddBrightness.call(this, control, objects, entity);

        //add hue and sat:
        _lightAdvancedAddHueAndSat.call(this, control, objects, entity);

        //add rgb. Will only happen, if no hue.
        _lightAdvancedAddRGBSingle.call(this, control, objects, entity);

        //add rgb as single states. Will only happen if no hue and no rgbSingle:
        _lightAdvancedAddRGB.call(this, control, objects, entity);
