...
function _lightAdvancedAddColorTemperature(states, objects, entity) {
    ...
    if (states.color_temp) {
        ...
        attribute.convert_to_kelvin = (tempObj.common.unit === 'K' || tempObj.common.unit === '°K');
        ...
        entity.attributes.max_mireds = tempObj.common.max || iobMaxValue;
        entity.attributes.min_mireds = tempObj.common.min || iobMinValue;
        if (attribute.convert_to_kelvin || entity.attributes.max_mireds > 1000) {
            attribute.convert_to_kelvin = true;
            entity.attributes.max_mireds = 1e6 / tempObj.common.max || iobMaxValue;
            entity.attributes.min_mireds = 1e6 / tempObj.common.min || iobMinValue;
        }
...
function _lightAdvancedAddBrightness(states, objects, entity) {
    ...
    if (states.brightness) {
        ...
        attribute.min = objects[attribute.getId].common.min || 0;
        attribute.max = objects[attribute.getId].common.max || 100;
...
function _lightAdvancedAddHueAndSat(states, objects, entity) {
    if (states.hue) {
        ...
        hue_attr.max = objects[states.hue].common.max || 360;
    ...
    if (states.saturation) {
        ...
        sat_attr.max = objects[states.saturation].common.max || 100;
...
function _lightAdvancedAddRGB(states, objects, entity) {
    if (states.red && states.green && states.blue) {
        ...
        red_attr.max = objects[states.red].common.max || 100;
        green_attr.max = objects[states.green].common.max || 100;
        blue_attr.max = objects[states.blue].common.max || 100;
...
async function fillLightEntityFromStates(states, objects, entity) {
    ...
    if (states.white) {
        white_attr.max = objects[white_attr.getId].common.max || 100;
    ...
    if (!entity.context.STATE.isBoolean) {
        ...
        entity.context.STATE.getParser = function (entity, attr, state) {
            ...
            entity.state = state.val > (stateObj.common.min || 0) ? 'on' : 'off';
            if (!entity.attributes.color_mode) {
                entity.attributes.color_mode = ONOFF;
            }
        };
