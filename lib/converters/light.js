...
const iobState2EntityState   = require('./genericConverter').iobState2EntityState;
...
async function readAttributesOnSwitchOn(entity) {
    ...
    for (const attr of Object.values(entity.context.ATTRIBUTES)) {
        ...
        if (id) {
            ...
            if (attr.getParser) {
                attributeValues[attr.attribute] = attr.getParser(entity, attr, state);
            } else {
                attributeValues[attr.attribute] = iobState2EntityState(entity, state ? state.val : null, attr.attribute);
            }
        }
    }
...
function _lightAdvancedAddState(states, objects, entity) {
    ...
    entity.context.STATE.getParser = (entity, attr, state) => {
        ...
        if (!state.val && oldState === 'on') {
            ...
            entity.attributes.effect = null;
            entity.attributes.color_mode = null;
            entity.attributes.brightness = null;
            entity.attributes.hs_color = null;
            entity.attributes.rgb_color = null;
            entity.attributes.xy_color = null;
        }
        if (state.val && oldState !== 'on') {
            readAttributesOnSwitchOn(entity);
        }
    };
