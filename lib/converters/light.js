...
function _lightAdvancedAddState(states, objects, entity) {
    ...
    if (states.state) {
        ...
        if (getState) {
            this._addID2entity(getState, entity);
        }
        return true;
    } else {
        return false;
...
function _lightAdvancedAddColorTemperature(states, objects, entity) {
    ...
    if (states.color_temp) {
        ...
        const attribute = {
            ...
            getParser: function (entity, attr, state) {
                ...
                if (targetCt > 1000 && !attr.convert_to_kelvin) {
                    ...
                    this.log.warn('Need kelvin conversion for ' + states.color_temp + ' and did not detect that in setup. Please set unit to "K" in object settings.');
                }
        ...
        if (tempObj.common.max) {
            ...
            this.adapter.log.warn(`no max value for light object '${states.color_temp}' defined -> using fallback max = '${iobMaxValue}'`);
        }
        if (tempObj.common.min) {
            ...
            this.adapter.log.warn(`no min value for light object '${states.color_temp}' defined -> using fallback min = '${iobMinValue}'`);
        }
        ...
        this._addID2entity(states.color_temp, entity);
    }
}
...
function _lightAdvancedAddHueAndSat(states, objects, entity) {
    if (states.hue) {
        ...
        this._addID2entity(states.hue, entity);
    }

    //add saturation as own attribute. Will update saturation values from ioBroker correctly.
    if (states.saturation) {
        if (!states.hue) {
            this.log.debug('Saturation present but no hue id found for ' + states.saturation + '. Hue won\'t work.');
            return;
        }
        const attribute = {
        ...
        entity.attributes.supported_features |= 0x10;
        entity.attributes.hs_color = [0,100];
        this._addID2entity(states.saturation, entity);
    } else if (states.hue) {
        this.log.debug('Hue present but no saturation id found for ' + states.hue + '. Saturation won\'t work.');
    }
}
...
async function _lightAdvancedAddRGBSingle(states, objects, entity) {
    ...
    if (states.rgb) {
        ...
        const rgbState = await this.adapter.getForeignStateAsync(states.rgb);
        ...
        entity.attributes.supported_features |= 0x10;
        entity.attributes.hs_color = [0,100];
        this._addID2entity(states.rgb, entity);
    }
}
...
function _lightAdvancedAddRGB(states, objects, entity) {
    ...
    if (states.red && states.green && states.blue) {
        ...
        entity.attributes.supported_features |= 0x10;
        entity.attributes.hs_color = [0,100];
        this._addID2entity(states.red, entity);
        this._addID2entity(states.green, entity);
        this._addID2entity(states.blue, entity);
        ...
        if (states.white) {
            ...
            this._addID2entity(states.white, entity);
        }
    }
}
...
async function fillLightEntityFromStates(states, objects, entity) {
    ...
    await _lightAdvancedAddState.call(this, states, objects, entity);

    //fill in color temperature stuff.
    await _lightAdvancedAddColorTemperature.call(this, states, objects, entity);

    //if there is a "BRIGHTNESS" control, use it to dim lamp.
    await _lightAdvancedAddBrightness.call(this, states, objects, entity);

    //add hue and sat:
    await _lightAdvancedAddHueAndSat.call(this, states, objects, entity);

    //add rgb. Will only happen, if no hue.
    await _lightAdvancedAddRGBSingle.call(this, states, objects, entity);

    //add rgb as single states. Will only happen if no hue and no rgbSingle:
    await _lightAdvancedAddRGB.call(this, states, objects, entity);
 ...
 * @returns {Promise<[entity]>}
 */
exports.processManualLight = async function(id, obj, entity) {
    ...
    return fillLightEntityFromStates.call(this,states, objects, entity);
};
