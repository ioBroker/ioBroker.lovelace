    ...
    if (entity.state === 'on') {
        for (const attr of attributesToNullOnOff) {
            if (entity.attributes[attr] === null || entity.attributes[attr] === undefined) {
                ...
                if (
                    entity.context.STATE.storedValues[attr] !== undefined &&
                    entity.context.STATE.storedValues[attr] !== null
                ) {
                    let val = entity.context.STATE.storedValues[attr];
                    if (Array.isArray(val)) {
                        //adapterData.log.debug('Restoring array ' + attr + ' with ' + JSON.stringify(val));
...
function _lightAdvancedAddBrightness(states, objects, entity) {
    ...
    if (states.brightness) {
        ...
        attribute.getParser = (entity, attr, state) => {
            ...
            targetAttributes.brightness = ((val - attr.min) / (attr.max - attr.min)) * 255;
...
function _lightAdvancedAddHueAndSat(states, objects, entity) {
    if (states.hue) {
        ...
        hue_attr.getParser = (entity, attr, state) => {
            ...
            targetAttributes.hs_color[0] = (val / attr.max) * 360;
    ...
    if (states.saturation) {
        ...
        if (!states.rgb_color && !states.red) {
            ...
            sat_attr.getParser = (entity, attr, state) => {
                ...
                targetAttributes.hs_color[1] = (val / attr.max) * 100;
                targetAttributes.color_mode = HS;
            };
        } else {
...
async function fillLightEntityFromStates(states, objects, entity) {
    utils.fillEntityFromStates(states, entity, objects); //already prefills attributes.
            ...
            white_attr.getParser = (entity, attr, state) => {
                ...
                if (!targetAttributes.rgbw_color) {
                    targetAttributes.rgbw_color = [
                        targetAttributes.rgb_color ? targetAttributes.rgb_color[0] : 255,
                        targetAttributes.rgb_color ? targetAttributes.rgb_color[1] : 255,
                        targetAttributes.rgb_color ? targetAttributes.rgb_color[2] : 255,
                    ];
                }
