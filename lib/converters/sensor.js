...
function createHumiditySensor(state, name, room, func, _obj, objects, forcedEntityId) {
    const entityHum = utils.processCommon(name, room, func, _obj || objects[state.id], 'sensor', forcedEntityId);
...
exports.processHumidity = function (id, control, name, room, func, _obj, objects, forcedEntityId) {
    const state = control.states.find(s => s.id && s.name === 'ACTUAL');
    if (state) {
        const entity = createHumiditySensor(state, name, room, func, _obj, objects, forcedEntityId);
        utils.addID2entity(state.id, entity);
        return [entity];
    }
    return [];
...
exports.processTemperature = function (id, control, name, room, func, _obj, objects, forcedEntityId) {
    ...
    if (state && state.id) {
        entity = utils.processCommon(name, room, func, _obj || objects[state.id], 'sensor', forcedEntityId);
        entity.context.STATE = {getId: state.id};
    ...
    if (state && state.id) {
        ...
        entityHum = createHumiditySensor(state, name, room, func, _obj, objects, humId);
