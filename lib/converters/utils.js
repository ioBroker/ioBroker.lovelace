...
/**
 * Function to test if a string is a valid entity id and a entity with that id exists.
 * @param {string} id
 * @returns {boolean}
 */
function isValidAndExistingEntityId(id) {
    if (/^[a-zA-Z0-9А-Яа-я_]+\.[a-zA-Z0-9А-Яа-я_]+$/.test(id)) {
...
function processCommon(name, room, func, obj, entityType, entity_id) {
    if (!name) {
        if (obj && obj.common && obj.common.name) {
            ...
            name = (obj && obj.common && obj.common.custom && obj.common.custom[entityData.adapter.namespace] && obj.common.custom[entityData.adapter.namespace].name) || _generateName(obj, entityData.lang);
        }
    }
    let idPart;
    ...
    for (const key of Object.keys(states)) {
        ...
        if (id) {
            ...
            if (key !== 'state' && key !== 'stateRead') {
                ...
                if (!attr) {
                    entity.context.ATTRIBUTES.push({attribute: key, getId: id});
                }
            }
        }
    }
...
exports.isValidAndExistingEntityId  = isValidAndExistingEntityId; //used to manage subscriptions.
