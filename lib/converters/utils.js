...
function _getObjectName(obj, lang) {
    ...
    if (!obj) {
        // No object, hence no name.
        return '';
    }

    if (!obj.common) {
        // No object.common, return ID or no name.
        return obj._id || '';
    }

    // We now know that we can access obj.common, try common.smartName or common.name.
    const candidates = [obj.common.smartName, obj.common.name];
    ...
    const found = candidates
      .reduce(function (acc, candidate) {
        ...
        if (typeof candidate === "object") {
          ...
          return candidate[lang] || candidate.en;
        }

        return candidate;
      }, undefined);

    return found || obj._id;
}
...
function _getFriendlyName(predefinedName, obj, room, func) {
    const candidates = [
        ...
        function() {
            // Fall back, use ioBroker object ID.
