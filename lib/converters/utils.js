...
function fillEntityIntoCaches(entity) {
    ...
    if (foundIndex !== -1) {
        entityData.log.warning(`Got duplicate for entity ${entity.entity_id}. Overwriting old value. Was for ${entityData.entities[foundIndex].context.id} and new one is for ${entity.context.id}`);
