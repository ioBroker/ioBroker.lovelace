...
function removeEntity(entity) {
    ...
    // this._entities       = []; done
    // this._entity2ID      = {}; done
    // this._ID2entity      = {}; <- no, done in removeIdFromEntities, which calls this function.
    // this._entityIconUrls = []; //stores icon urls that may be accessed without token.
    delete this._entity2ID[entity.entity_id];
    const foundIndex = this._entities.findIndex(x => x.entity_id === entity.entity_id);
    if (foundIndex !== -1) {
        this._entities.splice(foundIndex, 1);
    }
    if (entity.attributes.entity_picture) {
        const urlIndex = this._entityIconUrls.findIndex(entity.attributes.entity_picture);
        if (urlIndex !== -1) {
            this._entityIconUrls.splice(urlIndex, 1);
        }
    }
}
...
function removeIdFromEntities(id) {
    const affectedEntities = this._ID2entity[id];
    let changed = false;
    if (affectedEntities) {
            ...
            if (entity.context.ATTRIBUTES) {
                ...
                for (const attribute of entity.context.ATTRIBUTES) {
                    if (attribute.setId === id || attribute.getId === id) {
                        this.log.debug(`Removed attribute ${attribute.attribute} from ${entity.entity_id} because ${id} was deleted.`);
                        changed = true;
                    } else {
            ...
            if (entity.context.COMMANDS) {
                ...
                for (const command of entity.context.COMMANDS) {
                    if (command.setId === id || command.getId === id) {
                        this.log.debug(`Removed command ${command.service} from ${entity.entity_id} because ${id} was deleted.`);
                        changed = true;
                    } else {
...
exports._removeIdFromEntities = removeIdFromEntities;
